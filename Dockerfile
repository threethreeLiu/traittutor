# ============================================
# TraitTutor Multi-Stage Dockerfile
# ============================================
# This Dockerfile builds a production-ready image for TraitTutor
# containing both the FastAPI backend and Next.js frontend
#
# Build/run:
#   docker build -t traittutor:local .
#   docker run -p 127.0.0.1:3782:3782 -p 127.0.0.1:8001:8001 \
#     -v traittutor-data:/app/data traittutor:local
#
# Prerequisites:
...
# TRAITTUTOR_API_BASE_URL (exported by the entrypoint on every start).
...
# Labels
LABEL maintainer="TraitTutor Team" \
      description="TraitTutor: AI-Powered Personalized Learning Assistant"
...
# (system.json, default on), exported to TRAITTUTOR_SANDBOX_ALLOW_SUBPROCESS at
...
# (TRAITTUTOR_SANDBOX_RUNNER_URL), which build_backend() prefers.
...
# user). Each child (backend/frontend) is dropped to this `traittutor` user via
# the per-program `user=traittutor` directive, so the app processes stay
...
# traittutor user (UID 1000) via per-program `user=traittutor`; see the note on
...
WS_MAX_SIZE=$(python -c "from traittutor.services.config import get_ws_max_size; print(get_ws_max_size())" 2>/dev/null || echo 16777216)
exec python -m uvicorn traittutor.api.main:app --host ${BACKEND_HOST} --port ${BACKEND_PORT} --no-access-log --ws-max-size ${WS_MAX_SIZE}
EOF
...
# the configured backend at request time, reading TRAITTUTOR_API_BASE_URL
...
# Idempotent: re-chown /app/data so the unprivileged `traittutor` user (UID 1000)
...
# TRAITTUTOR_API_BASE_URL and TRAITTUTOR_AUTH_ENABLED are exported by the
...
# and rootless-keep-id runtimes; children drop to the traittutor user via
...
# `next dev` runs as the unprivileged traittutor user (via `user=traittutor` in
...
RUN mkdir -p /app/web/.next \
    && chown traittutor:traittutor /app/web /app/web/.next
...
RUN cat > /etc/supervisor/conf.d/programs.conf <<'EOF'
[program:backend]
command=/bin/bash -c "exec python -m uvicorn traittutor.api.main:app --host 0.0.0.0 --port ${BACKEND_PORT:-8001} --reload --no-access-log --ws-max-size $(python -c 'from traittutor.services.config import get_ws_max_size; print(get_ws_max_size())' 2>/dev/null || echo 16777216)"
directory=/app
user=traittutor
autostart=true
autorestart=true
