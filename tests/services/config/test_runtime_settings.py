...
def test_runtime_settings_creates_defaults_without_reading_dotenv(tmp_path: Path) -> None:
    ...
    assert service.load_auth(include_process_overrides=False)["enabled"] is True
...
def test_render_environment_uses_json_backed_runtime_names(monkeypatch, tmp_path: Path) -> None:
    ...
    # middleware). TRAITTUTOR_AUTH_ENABLED gates the login redirect;
    # TRAITTUTOR_API_BASE_URL is where the frontend server reaches the backend
    # (falls back to localhost:<backend_port> when no in-network / external base
    # is configured).
    assert env["TRAITTUTOR_AUTH_ENABLED"] == "true"
    assert env["TRAITTUTOR_API_BASE_URL"] == "http://localhost:8010"
...
def test_startup_ensure_creates_missing_runtime_jsons_with_defaults(
    ...
    from traittutor.services.config import model_catalog as model_catalog_module
    from traittutor.services.config import runtime_settings as runtime_settings_module
    ...
    assert _read_json(settings_dir / "auth.json")["enabled"] is True
...
def test_document_parsing_v1_to_v2_migration(tmp_path: Path) -> None:
    ...
    from traittutor.services.config.runtime_settings import _atomic_write_json
...
def test_compute_ws_max_size_floor_and_inflation() -> None:
    from traittutor.services.config.runtime_settings import compute_ws_max_size
