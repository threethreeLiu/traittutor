...
def _isolate_local_llm_overlay(monkeypatch, tmp_path):
    """Insulate JSON-catalog unit tests from any real config/models.local.yaml.

    The code-defined LLM overlay reads a global file on every load(); point it
    at a non-existent path so these tests exercise catalog mechanics in
    isolation (overlay is a no-op) rather than depending on the host config.
    """
    monkeypatch.setattr(
        "traittutor.services.models.local_catalog.local_models_path",
        lambda: tmp_path / "absent-models.local.yaml",
    )
...
def test_load_persists_normalized_active_ids(tmp_path: Path):
    ...
    # Normalize fixed the bogus active ids in the in-memory catalog.
    llm = catalog["services"]["llm"]
    assert llm["active_profile_id"] == "llm-profile-a"
    assert llm["active_model_id"] == "llm-model-a"

    # LLM models are code-defined (config/models.local.yaml) and overlaid on
    # load(); they are never persisted to the JSON catalog. On save() the llm
    # service is stripped back to the empty shell.
    saved = json.loads(catalog_path.read_text(encoding="utf-8"))
