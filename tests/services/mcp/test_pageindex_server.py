...
from traittutor.services.mcp import pageindex_server
from traittutor.services.mcp.config import MCPConfig, MCPServerConfig
from traittutor.services.mcp.manager import wrapped_tool_name
from traittutor.services.rag.pipelines.pageindex.config import (
    PageIndexConfig,
...
def _configured(monkeypatch, key: str = "sk-test") -> None:
    monkeypatch.setattr(
        "traittutor.services.rag.pipelines.pageindex.config.get_pageindex_config",
...
def _unconfigured(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "traittutor.services.rag.pipelines.pageindex.config.get_pageindex_config", _raise
    )
