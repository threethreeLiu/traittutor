...
from traittutor.services.session.pocketbase_store import PocketBaseSessionStore
...
def fake_pb(monkeypatch):
    client = _FakeClient()
    monkeypatch.setattr(
        "traittutor.services.pocketbase_client.get_pb_client", lambda: client, raising=True
    )
    return client
