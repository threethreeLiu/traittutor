...
from traittutor.core.tool_protocol import BaseTool, ToolDefinition, ToolResult
...
def _prepare(monkeypatch, docs: dict[str, dict[str, str]]) -> AgenticChatPipeline:
    ...
    monkeypatch.setattr("traittutor.services.mcp.get_mcp_manager", lambda: FakeManager())
    monkeypatch.setattr("traittutor.services.mcp.load_loaded_tools", lambda _sid: set())
    # Non-admin user without an MCP grant: fail-closed empty whitelist.
    monkeypatch.setattr("traittutor.multi_user.tool_access.allowed_mcp_tools", lambda: set())
