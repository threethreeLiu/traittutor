...
async def test_turn_runtime_replays_events_and_materializes_messages(
    ...
    monkeypatch.setattr("traittutor.services.llm.config.get_llm_config", lambda: SimpleNamespace())
    monkeypatch.setattr(
        "traittutor.services.session.context_builder.ContextBuilder", FakeContextBuilder
    )
    monkeypatch.setattr("traittutor.runtime.orchestrator.ChatOrchestrator", FakeOrchestrator)
    monkeypatch.setattr(
        "traittutor.book.context.build_book_context",
    ...
    monkeypatch.setattr(
        "traittutor.services.skill.get_skill_service",
        _fake_skill_service,
    )
    monkeypatch.setattr(
        "traittutor.services.persona.get_persona_service",
...
async def test_turn_runtime_persists_llm_selection_in_turn_snapshot(
    ...
    monkeypatch.setattr(
        "traittutor.services.model_selection.runtime.reset_llm_selection",
    ...
    monkeypatch.setattr("traittutor.services.skill.get_skill_service", _fake_skill_service)
    monkeypatch.setattr("traittutor.services.persona.get_persona_service", _fake_persona_service)
