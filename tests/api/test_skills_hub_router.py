...
catalog so the web panel can render it in TraitTutor's own UI (no iframe, no
...
from traittutor.services.skill import hub as hub_module
from traittutor.services.skill.hub import ClawHubProvider
...
def _mock_provider() -> ClawHubProvider:
                    ...
                    "skills": [
                        {
                            ...
                            "owner": {
                                "displayName": "TraitTutor",
                                "htmlUrl": "https://traittutor.info",
                            },
                        }
                    ]
                },
        ...
        if path == "/api/v1/skills/socratic-tutor":
            return httpx.Response(
                ...
                json={
                    ...
                    "owner": {"displayName": "TraitTutor", "htmlUrl": "https://traittutor.info"},
    ...
    return ClawHubProvider(
        "eduhub",
        base_url="https://eduhub.traittutor.info/api/v1",
        client=httpx.Client(transport=httpx.MockTransport(handler)),
    )


def _build_app() -> FastAPI:
    router = importlib.import_module("traittutor.api.routers.skills").router
    app = FastAPI()
...
def test_hub_catalog_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert data["web_url"] == "https://eduhub.traittutor.info"
    row = data["skills"][0]
    assert row["slug"] == "socratic-tutor"
    assert row["downloads"] == 8 and row["stars"] == 2
    assert row["owner"] == "TraitTutor"
...
def test_hub_detail_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert data["web_url"] == "https://eduhub.traittutor.info/skills/socratic-tutor"
