...
from traittutor.api import main as api_main


def test_cors_allows_remote_http_origins_when_auth_disabled(
    monkeypatch,
) -> None:
    # Auth is enabled by default now, so the permissive-CORS branch must be
    # exercised by explicitly disabling auth (the local/single-user mode that
    # still wants remote Docker/LAN origins out of the box).
