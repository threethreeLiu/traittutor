...
from traittutor.core.errors import (
    ConfigurationError,
    TraitTutorError,
...
class TestErrors:
    def test_base_error_str_without_details(self) -> None:
        err = TraitTutorError("boom")
        assert str(err) == "boom"
        assert err.details == {}

    def test_base_error_str_with_details(self) -> None:
        err = TraitTutorError("boom", details={"key": "val"})
        assert "key" in str(err)

    def test_hierarchy(self) -> None:
        assert issubclass(ConfigurationError, TraitTutorError)
        assert issubclass(ValidationError, TraitTutorError)
        assert issubclass(ServiceError, TraitTutorError)
