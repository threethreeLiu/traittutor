...
from traittutor.api.routers.auth import LoginRequest, RegisterRequest
...
def test_register_accepts_legacy_email_field_and_normalizes_email_case() -> None:
    request = RegisterRequest(email="  Learner@Example.com  ", password="password1234")

    assert request.username == "learner@example.com"


def test_login_accepts_legacy_email_field() -> None:
    request = LoginRequest(email=" Learner@Example.com ", password="x")
