...
``traittutor init`` walks the user through a four-step wizard (ports → LLM →
...
review panel) lives in :mod:`traittutor_cli.init_wizard`. This module is
...
from traittutor.runtime.home import TRAITTUTOR_HOME_ENV, get_runtime_home

from . import init_wizard as wiz


def _reset_runtime_singletons() -> None:
    """Drop cached service instances so the new TRAITTUTOR_HOME takes effect.

    ``traittutor init`` may pass ``--home`` to target a different workspace; the
...
def _embedding_step(
    ...
    from traittutor.services.config.embedding_endpoint import (
        EMBEDDING_PROVIDER_LABELS,
        normalize_embedding_endpoint_for_display,
    )
    from traittutor.services.config.provider_runtime import EMBEDDING_PROVIDERS
...
def run_init(*, cli_only: bool = False, home: str | Path | None = None) -> None:
    ...
    os.environ[TRAITTUTOR_HOME_ENV] = str(runtime_home)
    _reset_runtime_singletons()

    from traittutor.runtime.banner import labels_for, print_banner, resolve_language
    from traittutor.services.config import get_model_catalog_service, get_runtime_settings_service
