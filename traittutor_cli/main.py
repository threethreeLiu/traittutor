...
def run_capability(
    ...
    maybe_run(run_turn_and_render(app=TraitTutorApp(), request=request, fmt=fmt))
...
def start(
    ...
    from traittutor.runtime.launcher import start as start_web
...
def serve(
    ...
    """Start the TraitTutor API server."""
    import asyncio
    import sys

    set_mode(RunMode.SERVER)
    if port is None:
        from traittutor.services.setup import get_backend_port
    ...
    try:
        ...
        console.print(
            ...
            "Run: pip install -U traittutor"
        )
        raise typer.Exit(code=1)

    from traittutor.services.config import get_ws_max_size
