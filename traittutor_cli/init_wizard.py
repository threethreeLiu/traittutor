"""Interactive setup helpers used by ``traittutor init``.
...
from traittutor.services.llm.config import get_token_limit_kwargs
from traittutor.services.provider_registry import PROVIDERS, ProviderSpec, find_by_name
...
# ``traittutor.services.config.provider_runtime``. Adding a new featured entry
...
# ``traittutor.services.config.provider_runtime``. Each entry below describes
...
def _all_providers_except(featured: set[str]) -> list[ProviderSpec]:
    ...
    return [
        ...
        if spec.name not in featured and not spec.is_oauth  # OAuth flows use `traittutor login`
    ]
