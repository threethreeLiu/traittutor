...
CLI-only TraitTutor distribution. It installs the `traittutor` command and the
...
FastAPI/Uvicorn server dependencies used by `traittutor start`.
...
the `traittutor` command at these source files.
