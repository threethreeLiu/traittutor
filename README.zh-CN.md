...
TraitTutor 仓库代码采用 [Apache-2.0](LICENSE) 协议；Python 依赖以 `pyproject.toml` 为准，前端依赖以 `web/package.json` 为准，第三方依赖各自保留原有许可证。模型网关可以连接本地配置的商业或开源模型服务，但仓库不包含服务商密钥、模型权重、私有用户材料或专有服务输出。

## 路线图与落地计划

当前版本聚焦可运行的“目标 → 学习路径”、材料分析、练习证据回流和可解释下一步。近期优先级包括生成评测门槛、持久化 TTS 资产，以及覆盖“材料 → 练习 → 产物追问”的浏览器 smoke 测试。
