export type PendingGenerationTask = {
  generationId: string
  status?:
    'queued' | 'running' | 'needs_review' | 'failed' | 'cancelled' | 'interrupted' | 'discarded'
  surface?: 'chat' | 'study'
  sessionId?: string
  createdAt?: string
  packId?: string
      ...
      return [
        {
          ...
          ...(task.surface === 'chat' || task.surface === 'study'
            ? { surface: task.surface }
            : {}),
          ...(typeof task.sessionId === 'string' && task.sessionId
...
export function writePendingGenerationTask(scope: string, task: PendingGenerationTask): void {
  ...
  const next = [
    ...existing.filter(entry => entry.generationId !== task.generationId),
    { ...previous, ...task },
  ]
...
export function removePendingGenerationTask(scope: string, generationId: string): void {
  const next = readPendingGenerationTasks(scope).filter(
