...
//
// Single source of truth for raw-HTML rendering: the text-level escape here
// and the hast-level sanitize schema in `markdown-sanitize.ts` both read
// this set. Keep the two defenses in sync through it, never by duplicating
// the list.
...
/**
 * Read-only view of {@link ALLOWED_HTML_TAGS} for the hast-level sanitize
 * schema in `markdown-sanitize.ts`.
 */
export const MARKDOWN_ALLOWED_HTML_TAG_NAMES: readonly string[] =
  Array.from(ALLOWED_HTML_TAGS);
