 ...
 *
 * Returns ``null`` until at least one valid quiz event has arrived.
 */
...
export function extractQuizQuestions(
  ...
  const parsed: QuizQuestion[] = results.map((item) => {
    const qa = item.qa_pair;
    if (!qa || typeof qa !== "object" || !("question" in qa) || !qa.question) {
      throw new Error("Invalid canonical quiz result: qa_pair is required");
    }
    const canonical = qa as Record<string, unknown>;
    const question: QuizQuestion = {
      question_id: String(canonical.question_id ?? ""),
      question: String(canonical.question ?? ""),
      question_type: normalizeQuizQuestionType(canonical.question_type),
      options: canonical.options as Record<string, string> | undefined,
      difficulty: canonical.difficulty ? String(canonical.difficulty) : undefined,
      concentration: canonical.concentration ? String(canonical.concentration) : undefined,
      knowledge_context:
        canonical.metadata &&
        typeof canonical.metadata === "object" &&
        "knowledge_context" in canonical.metadata &&
        canonical.metadata.knowledge_context
          ? String(canonical.metadata.knowledge_context)
          : undefined,
    };
    return question;
  });
