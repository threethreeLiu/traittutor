...
export async function listNotebooks(): Promise<NotebookSummary[]> {
  ...
  const data = await expectJson<{ notebooks: NotebookSummary[] }>(response)
...
export async function getNotebook(notebookId: string): Promise<NotebookDetail> {
  ...
  return expectJson<NotebookDetail>(response)
}
...
export async function createNotebook(payload: {
  ...
  const data = await expectJson<{ notebook: NotebookSummary }>(response)
...
export async function deleteNotebookRecord(notebookId: string, recordId: string): Promise<void> {
  const response = await apiFetch(apiUrl(`/api/v1/notebook/${notebookId}/records/${recordId}`), {
    method: 'DELETE',
  })
  expectOk(response)
}
...
export async function upsertNotebookEntry(data: {
  ...
  turn_id?: string
  question_id: string
  /** Learner answer only; the server resolves the stored question/key. */
  answer?: string
  attempt_id?: string
  /** @deprecated ignored at the public boundary; server-held only. */
  question: string
  question_type?: string
  options?: Record<string, string>
  ...
  const response = await apiFetch(apiUrl('/api/v1/question-notebook/entries/upsert'), {
    ...
    body: JSON.stringify({
      ...
      answer: data.answer ?? data.user_answer ?? '',
      attempt_id: data.attempt_id ?? '',
      user_answer_images: data.user_answer_images,
    }),
  })
  return expectJson<NotebookEntry>(response)
}
...
export async function removeEntryFromCategory(entryId: number, categoryId: number): Promise<void> {
  const response = await apiFetch(
...
export async function renameCategory(categoryId: number, name: string): Promise<void> {
  const response = await apiFetch(apiUrl(`/api/v1/question-notebook/categories/${categoryId}`), {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name }),
  })
