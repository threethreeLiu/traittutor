...
import { buildMarkdownSanitizeSchema } from "@/lib/markdown-sanitize";
import {
  ...
  stripLeadingHeadingHashes as stripLeadingHashes,
...
export default function RichMarkdownRenderer({
  ...
  useEffect(() => {
    ...
    async function loadPlugins() {
      ...
      if (allowHtml) {
        // rehype-raw parses embedded HTML from untrusted content (model
        // output, extracted material). It must never run unsanitized: the
        // schema strips event handlers, javascript:/data: URLs, scripts and
        // any tag outside the shared allowlist before the tree reaches React.
  ...
  const traceComponents: Record<string, React.ComponentType<any>> = {
    ...
    h4: ({ node: _node, children }: any) => (
      <p className="mb-1.5 font-semibold">{children}</p>
    ),
    h5: ({ node: _node, children }: any) => (
      <p className="mb-1.5 font-semibold">{children}</p>
    ),
    h6: ({ node: _node, children }: any) => (
      <p className="mb-1.5 font-semibold">{children}</p>
    ),
    strong: ({ node: _node, children }: any) => (
      <strong className="font-semibold text-[var(--foreground)]">
        {children}
      </strong>
    ),
    em: ({ node: _node, children }: any) => <em className="italic">{children}</em>,
    a: ({ node: _node, children }: any) => (
    ...
    li: ({ node: _node, ...props }: any) => <li className="my-0.5 pl-0" {...props} />,
    ...
    option: () => null,
    textarea: () => null,
    details: ({ node: _node, children }: any) =>
      hasRenderableDetailsBody(children) ? <div>{children}</div> : null,
    summary: ({ node: _node, children }: any) =>
  ...
  const sanitizeSchema = useMemo(() => buildMarkdownSanitizeSchema(), []);
  const rehypePlugins = useMemo(() => {
    const p: Array<any> = [];
    if (allowHtml && plugins.rehypeRaw && plugins.rehypeSanitize) {
      p.push(plugins.rehypeRaw as never);
      // Order matters: sanitize the parsed raw HTML *before* KaTeX injects
      // its generated markup, so math rendering is not stripped while the
      // untrusted source is still fully whitelisted.
      p.push([plugins.rehypeSanitize, sanitizeSchema] as never);
    }
