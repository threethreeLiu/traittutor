...
export default function AccountPrivacyPage() {
  ...
  useEffect(() => {
    void fetchAuthStatus().then(async (status) => {
      ...
      try { setProfile(await getProfile()); } catch { setError("无法读取账户信息"); }
    });
  }, [router]);
  ...
  return <div className="mx-auto max-w-xl"><SettingsPageHeader title="账户与隐私" description="管理登录凭据和你在 TraitTutor 中保留的数据。" />
    ...
    <section className="mt-4 flex flex-col gap-3 rounded-lg border border-[var(--border)] bg-[var(--card)] p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-semibold">退出登录</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">结束此设备上的会话。</p></div><button onClick={() => void signOut()} className="inline-flex h-9 w-fit items-center gap-2 rounded-md border border-[var(--border)] px-3 text-sm"><LogOut size={15} />退出</button></section>
