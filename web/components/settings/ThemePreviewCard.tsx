...
type Palette = {
  ...
  muted: string;
  border: string;
  // True for themes with a soft atmospheric gradient.
  atmospheric?: boolean;
};
...
// Renders a miniature TraitTutor UI mockup in the given theme's palette —
...
function MiniPreview({ palette }: { palette: Palette }) {
  const { bg, fg, card, primary, muted, border, atmospheric } = palette;
  return (
    <svg viewBox="0 0 160 96" className="block h-full w-full" aria-hidden>
      ...
      {atmospheric && (
        <>
          <defs>
            <radialGradient id="teal-shine" cx="20%" cy="0%" r="80%">
