...
const DIAGNOSTICS_RESULTS_KEY = 'traittutor.settings.diagnosticsResults.v1'
...
function isCatalog(value: unknown): value is Catalog {
  ...
  return SERVICE_NAMES.every(service => {
    const entry = services[service]
    return (
      isRecord(entry) &&
      (entry.active_profile_id === null || typeof entry.active_profile_id === 'string') &&
...
function parseSettingsPayload(value: unknown): SettingsPayload {
  if (!isRecord(value) || !isRecord(value.ui)) {
    throw new Error('Settings response is missing ui settings')
  }
  const { theme, language } = value.ui
  if (!['light', 'dark', 'snow', 'teal'].includes(String(theme))) {
    throw new Error('Settings response contains an invalid theme')
  }
  if (language !== 'en' && language !== 'zh') {
    throw new Error('Settings response contains an invalid language')
  }
  if (value.catalog !== undefined && !isCatalog(value.catalog)) {
    throw new Error('Settings response contains an invalid catalog')
  }
  if (
    value.providers !== undefined &&
    (!isRecord(value.providers) || !hasProviderArrays(value.providers))
  ) {
    throw new Error('Settings response contains invalid provider choices')
  }
  return value as SettingsPayload
}

function hasProviderArrays(providers: Record<string, unknown>): boolean {
  return SERVICE_NAMES.every(service => Array.isArray(providers[service]))
}
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      // Validate the complete response before committing any part of it. A
      // partial/malformed payload must not replace the safe default catalog
      // and then crash unrelated Settings pages during render.
      const payload = parseSettingsPayload(await settingsResponse.json())
      if (payload.catalog) {
  ...
  const hasUnsavedChanges = useMemo(() => {
    if (hasDirtyExtension) return true
    if (catalogEditable !== true) return false
    // LLM is code-defined (config/models.local.yaml) and rendered read-only on
    // the settings page. Its draft can diverge from the catalog only by
    // switching the viewed profile/model, which is a view action, not an edit
    // — exclude it so browsing the read-only LLM page never lights up the
    // global "unsaved changes" state or enables Save/Apply on other pages.
    for (const key of Object.keys(catalog.services) as ServiceName[]) {
      if (key === 'llm') continue
      if (JSON.stringify(catalog.services[key]) !== JSON.stringify(draft.services[key])) {
        return true
      }
    }
    return false
