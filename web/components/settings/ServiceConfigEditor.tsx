...
export function ServiceConfigEditor({ service }: { service: ServiceName }) {
  ...
  // LLM models are code-defined in config/models.local.yaml and surfaced via
  // `traittutor models sync-cc-switch`. The settings page renders them
  // read-only — no add/edit/delete of profiles, models, or keys, and no
  // save/apply. Every other service stays fully editable.
  const readOnly = service === 'llm'
  ...
  return (
    <div data-tour={`tour-${service}`} className="space-y-5">
      {readOnly && (
        <div className="flex items-start gap-2 rounded-xl border border-[var(--border)]/60 bg-[var(--muted)]/30 px-4 py-3 text-[12.5px] leading-relaxed text-[var(--muted-foreground)]">
          <Info className="mt-0.5 h-3.5 w-3.5 shrink-0" />
          <span>
            {t(
              'Models are code-defined in config/models.local.yaml. Use `traittutor models sync-cc-switch` to manage them.'
            )}
          </span>
        </div>
      )}
              ...
              {draft.services[service].profiles.map(profile => {
                ...
                return (
                  <div
                    ...
                    onDoubleClick={readOnly ? undefined : () => startProfileRename(profile)}
                    onKeyDown={e => {
                      if (isEditing) return
                    ...
                    title={isEditing || readOnly ? undefined : t('Double-click to rename')}
                        ...
                        <div key={model.id} className="min-w-0">
                          {editingModelId === model.id ? (
                            ...
                            <button
                              ...
                              onDoubleClick={readOnly ? undefined : () => startModelRename(model)}
                              title={readOnly ? undefined : t('Double-click to rename')}
  ...
  return (
    <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-2 rounded-lg border border-[var(--border)] bg-[var(--muted)]/30 px-3 py-2 sm:col-span-2">
      ...
      <button
        ...
        onClick={onApply}
        disabled={readOnly}
        className="shrink-0 rounded-md border border-[var(--border)] bg-[var(--background)] px-2.5 py-1 text-[11.5px] font-medium text-[var(--foreground)] transition-colors hover:border-[var(--foreground)] disabled:cursor-not-allowed disabled:opacity-40"
      >
        {t('Apply')}
      </button>
    </div>
...
function ProfileFields({
  ...
  // Disabling inputs (LLM read-only view) needs a slightly muted, non-pointer
  // cursor on text inputs/selects; the native select class already carries
  // disabled variants.
  const fieldInputClass = readOnly ? `${inputClass} cursor-not-allowed opacity-70` : inputClass
      ...
      {fields.baseUrl && (
        <div className="sm:col-span-2">
          ...
          {service === 'embedding' && (
            <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
              {t(
                'Embedding requests are sent to this URL exactly; TraitTutor does not append /embeddings or /api/embed at request time.'
              )}
            </p>
          )}
