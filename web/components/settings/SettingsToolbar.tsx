...
export function SettingsToolbar() {
  ...
  // LLM models are code-defined; the LLM settings page is read-only, so the
  // global Save Draft / Apply actions are hidden there (no PUT /catalog or
  // POST /apply should fire from that page). Other pages keep both actions.
  const isLlmReadOnlyPage = pathname.startsWith('/settings/llm')
  const {
    catalogEditable,
    hasUnsavedChanges,
    saving,
  ...
  if (catalogEditable !== true) {
    ...
    return (
      <div className="flex items-center justify-end px-1 py-2">
        <p className="text-[12px] text-[var(--primary)] animate-fade-in">{toast}</p>
      </div>
    )
  }
    ...
    <div className="flex items-center justify-between gap-3 px-1 py-2">
      ...
      >
        {toast ? (
          ...
          <>
            {t('Saved to')}{' '}
            <span className="font-mono text-[var(--foreground)]/65">{storagePath}</span>
          </>
        ) : (
          t('All changes saved')
        )}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={saveCatalog}
          disabled={saving || !hasUnsavedChanges || isLlmReadOnlyPage}
          hidden={isLlmReadOnlyPage}
        ...
        <button
          ...
          disabled={applying || isLlmReadOnlyPage}
          hidden={isLlmReadOnlyPage}
