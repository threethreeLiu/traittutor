 ...
 * (look, controls, reference popup, KB picker, attachments, LLM selector,
 * picker dialogs) matches the main chat composer exactly.
 *
 * Self-contained: loads its own KB / LLM lists, owns drag-and-drop +
 * attachment state, mounts the reference pickers internally.
 */
...
function FollowupChatComposerImpl({ context }: FollowupChatComposerProps) {
  ...
  // Click-outside handlers for menu chrome (capability / references).
  useEffect(() => {
    if (!capMenuOpen && !referenceMenuOpen) return;
