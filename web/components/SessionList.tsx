            ...
            >
              ...
              {isEditing ? (
                <input
                  ...
                  className="min-w-0 flex-1 rounded border border-[var(--border)] bg-[var(--background)] px-1.5 py-px text-[14.5px] text-[var(--foreground)] outline-none focus:ring-1 focus:ring-[var(--primary)]/40"
                />
              ) : isPlaceholderTitle(session.title) ? (
                <span
                  className={`dt-breathing-text min-w-0 flex-1 truncate text-[14.5px] italic text-[var(--muted-foreground)] ${active ? "font-medium" : ""}`}
                >
                  {placeholderLabel}
                </span>
              ) : (
                <span
                  className={`min-w-0 flex-1 truncate text-[14.5px] ${active ? "font-medium" : ""}`}
                >
                  {session.title}
                </span>
              )}
