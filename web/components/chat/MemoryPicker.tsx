...
interface MemoryPickerProps {
  open: boolean;
  initialFiles: ChatMemoryFile[];
  onClose: () => void;
  onApply: (files: ChatMemoryFile[]) => void;
}
...
export default function MemoryPicker({
  ...
  const [selected, setSelected] = useState<ChatMemoryFile[]>(initialFiles);
  ...
  const toggle = (key: ChatMemoryFile) => {
    setSelected((prev) =>
