...
export default function FilePreviewDrawer({
  ...
  return (
    <div
      ...
      className={`fixed right-0 top-0 z-[75] flex h-full w-full flex-col border-l border-[var(--border)] bg-[var(--card)] transition-transform ease-out lg:w-[min(560px,92vw)] ${
...
const PreviewBody = memo(function PreviewBody({
  ...
  // Office documents may be rendered from the canonical extracted-text fields
  // even when their binary preview URL is unavailable.
  if (kind === "office-text") {
    return (
      <OfficeTextPreview
        filename={filename}
  ...
  // Every other canonical preview requires a fetchable URL.
  if (!previewUrl) {
    return <FallbackPreview filename={filename} url={null} reason="unsupported" />;
  }
