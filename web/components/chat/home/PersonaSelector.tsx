...
export default function PersonaSelector({
  ...
  const filtered = useMemo(() => {
    ...
    return personas.filter(
      (p) =>
        personaSearchText(p, i18n.language).includes(q),
    );
  }, [i18n.language, personas, query]);

  const defaultLabel = t("Default");
  const label = value
    ? presentPersona(personas.find((persona) => persona.name === value) ?? {
        name: value,
        description: "",
        source: "user",
        read_only: false,
        ...
        >
          ...
          <div className="max-h-[280px] overflow-y-auto py-1">
            ...
            {filtered.map((persona) => {
              ...
              const display = presentPersona(persona, i18n.language);
              return (
                <button
                  key={persona.name}
                  type="button"
