...
/**
 * v3 ``ask_user`` payload. Mirrors ``traittutor.tools.ask_user.AskUserPayload``.
 *
 ...
 * carries this canonical shape from transport through rendering.
 */
      ...
      resolution = {
        ...
        answers: answersRaw
          .map((entry) => {
            ...
            const qid = String(obj.questionId || "").trim();
            if (!qid) return null;
...
const InteractiveAskUserCard = memo(function InteractiveAskUserCard({
  ...
  const answeredCount = useMemo(
    () => payload.questions.filter((q) => (answers[q.id] ?? "").trim()).length,
    [payload.questions, answers],
  );
  ...
  const submitLabel = allAnswered
    ...
    : answeredCount === 0
      ? t("Skip for now")
      : t("Submit and skip {{count}}", { count: unansweredCount });
  ...
  return (
    <section
      className="mt-3 border-y border-[var(--border)]/70 py-4"
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3
            id={promptId}
            className="text-[13px] font-semibold leading-snug text-[var(--foreground)] [text-wrap:pretty]"
          >
            {payload.intro || t("Please answer to continue.")}
          </h3>
          <div
            aria-live="polite"
            className="mt-1 text-[11.5px] leading-5 text-[var(--muted-foreground)]"
          >
            {submitted
              ? t("Sending your answers…")
              : totalQuestions > 1
                ? t("{{answered}} of {{count}} answered — choose a question to continue.", {
          ...
          {payload.questions.map((q, idx) => {
            ...
            return (
              <button
                ...
                aria-label={`${idx + 1}. ${q.header || q.prompt}${answered ? ` — ${t("Answered")}` : ""}`}
                className={
                  "flex min-h-10 items-center gap-1.5 rounded-full border px-3 py-2 text-[11.5px] font-medium transition-all " +
                  (isActive
      ...
      <div className="mt-4 flex flex-col gap-3 border-t border-[var(--border)]/60 pt-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 flex-1 items-center">
          {totalQuestions > 1 && activeIdx > 0 ? (
            <button
              ...
              disabled={submitted}
              className="inline-flex min-h-10 items-center gap-1 rounded-xl border border-[var(--border)] bg-transparent px-3 py-2 text-[12px] font-medium text-[var(--foreground)] transition-colors hover:border-[var(--foreground)]/30 hover:bg-[color-mix(in_srgb,var(--foreground)_4%,transparent)] disabled:cursor-not-allowed disabled:opacity-40"
            >
              <ChevronLeft size={14} strokeWidth={2} />
              <span>{t("Previous question")}</span>
            </button>
          ) : (
            <div className="text-[11.5px] leading-5 text-[var(--muted-foreground)]">
              {allAnswered
                ? t("All questions answered.")
                : t("{{count}} unanswered. The submit button will skip them.", {
        ...
        {totalQuestions > 1 && activeIdx < totalQuestions - 1 ? (
          <button
            ...
            disabled={submitted}
            className="inline-flex min-h-10 items-center justify-center gap-1 rounded-xl bg-[var(--primary)] px-4 py-2 text-[12px] font-medium text-[var(--primary-foreground)] hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
          >
          ...
          <button
            ...
            disabled={submitted}
            className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-2 text-[12px] font-medium text-[var(--primary-foreground)] hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40 sm:ml-auto"
          >
            {submitted ? <Loader2 aria-hidden="true" size={14} className="animate-spin" /> : null}
            {submitted ? t("Sending your answers…") : submitLabel}
          </button>
        )}
      </div>
    </section>
...
const QuestionBody = memo(function QuestionBody({
  ...
  return (
    <>
      <div
        id={headingId}
        className="mt-4 text-[14px] font-medium leading-snug text-[var(--foreground)] [text-wrap:pretty]"
      >
        {question.prompt}
      ...
      {question.options.length > 0 ? (
        <div className="mt-2 flex flex-col gap-2" role="group" aria-labelledby={headingId}>
          {question.options.map((option, idx) => {
            ...
            return (
              <button
                ...
                className={
                  "group flex min-h-11 w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition-all " +
                  (isPicked
        ...
        <div className="mt-1.5">
          {customSelected ? (
            ...
            <button
              ...
              disabled={locked}
              className="flex min-h-11 w-full items-center gap-3 rounded-xl border border-dashed border-[var(--border)] bg-transparent px-3 py-2.5 text-left text-[13px] text-[var(--muted-foreground)] transition-colors hover:border-[var(--foreground)]/30 hover:bg-[color-mix(in_srgb,var(--foreground)_3%,transparent)] hover:text-[var(--foreground)] disabled:cursor-not-allowed disabled:opacity-60"
            >
