...
function getModeBadgeLabel(capability?: string | null): string {
  ...
  if (capability === "deep_solve") return "Solver";
  if (capability === "learning_exploration") return "Learning Exploration";
  if (capability === "knowledge_diagram") return "Knowledge Diagram";
  if (capability === "humanizer") return "Humanizer";
  ...
  if (capability === "mastery_path") return "Learning Profile";
  return capability;
}
...
/** "TraitTutor_Introduction.pdf" → "TraitTutor Introduction" — the card title
...
const AssistantMessage = memo(function AssistantMessage({
   ...
   * bubble). Replies use the canonical per-question answer list.
   */
  ...
  return (
    <>
      {/* Activity block pinned to the TOP: the status header
          ("TraitTutor Exploring… · 8s" → "TraitTutor responded. · 10s") with
          the exploring trace nested beneath it — expanded while TraitTutor is
...
const UserMessage = memo(function UserMessage({
  ...
  // snapshot's source references — rendered as one collapsed tree under
...
export const ChatMessageList = memo(function ChatMessageList({
   ...
   * the canonical per-question answer list.
   */
  onSubmitUserReply?: (reply: {
    answers: Array<{ questionId: string; text: string }>;
  }) => void;
