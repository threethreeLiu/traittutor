...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "TraitTutor Reflecting…"
...
// Whether a call's public events carry anything worth a trace row: a tool
...
function TraceRowBody({
  ...
  const plainSummaryEvents = summaryProgressEvents;
...
function hasExpandableContent(
  ...
  return (
    ...
    inlineSources.length > 0 ||
    (group === "react_round" && Boolean(observationText))
  );
}
...
function TraceRowItem({
  ...
  // Public narration and planning summaries stream inline. Provider reasoning
  // chunks are deliberately excluded from learner-facing trace output.
  const isThinking = isChatRound || kind === "llm_planning";
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "TraitTutor Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  else if (mode === "responded") modeLabel = t("TraitTutor responded.");
 ...
 * reasoning trace once TraitTutor stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("TraitTutor Exploring… · 8s", settling to "TraitTutor responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while TraitTutor is still reasoning /
