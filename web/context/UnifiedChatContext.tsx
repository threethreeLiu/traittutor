...
// Optimistic (negative) message ids for rows the client creates before the
// server assigns real ids. ``-Date.now()`` alone can collide when several
// rows are created within the same millisecond — the user bubble and the
// assistant streaming placeholder are dispatched back-to-back — which breaks
// React list keys and the id-keyed branch maps in ``message-branches``.
// The monotonic counter keeps ids strictly decreasing (preserving the
// "more negative = more recent" sibling ranking) and guarantees uniqueness
// even under clock skew, since it only ever moves downward.
let lastOptimisticMessageId = 0

function nextOptimisticMessageId(): number {
  lastOptimisticMessageId = Math.min(lastOptimisticMessageId - 1, -Date.now())
  return lastOptimisticMessageId
}
...
interface ChatContextValue {
   ...
   *
   * Accepts the canonical per-question answer list.
   */
  submitUserReply: (reply: {
...
export function UnifiedChatProvider({
  ...
  const cancelStreamingTurn = useCallback(() => {
    ...
    if (!session) return;
    const runner = runnersRef.current.get(key);
    // Connection close is the protocol-level cancellation signal. The server
    // owns the turn id started on this socket and cooperatively cancels its
    // Gateway execution during disconnect cleanup.
    if (runner?.client.connected) {
      runner.client.disconnect();
      runnersRef.current.delete(key);
    }
  ...
  const submitUserReply = useCallback(
    (reply: { answers: Array<{ questionId: string; text: string }> }) => {
      const currentState = stateRef.current;
      const key = currentState.selectedKey;
      if (!key) return;
      const session = currentState.sessions[key];
