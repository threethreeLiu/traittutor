# TraitTutor Containerization

This document covers deploying TraitTutor from a container image: the
...
The published `ghcr.io/<your-org>/traittutor` image runs both the FastAPI
...
- `web/proxy.ts` catches `/api/*` and `/ws/*` and rewrites them to
  `TRAITTUTOR_API_BASE_URL` at request time. That env var is set by the
...
- `supervisord` runs as root (PID 1) and drops each program (backend,
  frontend) to a non-root `traittutor` user (UID 1000) via its per-program
...
memory, and knowledge bases persist in the `traittutor-data` named volume.
...
- **Only `3782` needs to be published.** The browser talks exclusively to
  ...
  middleware (`web/proxy.ts`), which reads `TRAITTUTOR_API_BASE_URL`
...
- **Detached:** add `-d`, then `docker logs -f traittutor` to follow,
  `docker stop traittutor` to stop, `docker rm traittutor` before reusing
  the name. The `traittutor-data` volume keeps your settings and workspace
  across restarts.
...
(`https://traittutor.example.com`), and the in-container Next.js middleware
...
on the host (inside the `traittutor-data` volume — `docker volume inspect
traittutor-data` to find its mountpoint) and set the in-network address the
...
`TRAITTUTOR_API_BASE_URL` for `proxy.ts` (precedence: `next_public_api_base`,
then `next_public_api_base_external`, then `http://localhost:8001`). Note
that because the proxy is **server-side**, `TRAITTUTOR_API_BASE_URL` is the
...
TraitTutor permits normal HTTP/HTTPS browser origins by default. With
...
point. It pulls the same `ghcr.io/<your-org>/traittutor:latest` image and
...
podman compose -f compose.yaml logs -f traittutor
```
...
- **`userns_mode: keep-id`.** The container's UID 0 maps to your host
  UID; the container's UID 1000 (the `traittutor` user inside the image)
...
podman run --rm -d --name traittutor \
...
non-root `traittutor` user (UID 1000) — `podman exec traittutor ps -o user,pid,comm`
shows the `uvicorn`/`node` children as `traittutor`. `supervisord` itself
...
programs are dropped to `traittutor` via the per-program `user=` directives.
...
- **`system.json` → `next_public_api_base`** (in-network) and
  **`next_public_api_base_external`** (cloud/external override). The
  entrypoint reads these and exports `TRAITTUTOR_API_BASE_URL`, which
...
service up alongside the main `traittutor` service. With it running, the
...
always a backend that failed to start (check `docker logs traittutor` for the
`[program:backend]` lines) or a wrong `TRAITTUTOR_API_BASE_URL` in a split
...
`ghcr.io/<your-org>/traittutor:latest` again.
...
- The image drops privileges to a non-root `traittutor` user (UID 1000)
...
- Auth (`data/user/settings/auth.json` → `auth_enabled = true`) gates
  ...
  `TRAITTUTOR_AUTH_ENABLED` (exported by the entrypoint on every start)
