#!/usr/bin/env python
"""Safely update a local TraitTutor git checkout.
...
def dependency_hints(changed_files: list[str]) -> list[str]:
    hints: list[str] = []
    if any(path == "pyproject.toml" for path in changed_files):
...
def run_update(repo_root: Path, *, assume_yes: bool) -> int:
    ...
    print("Restart TraitTutor if it is currently running.")
    return 0


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch, review, and fast-forward update a local TraitTutor checkout."
    )
    parser.add_argument(
        "--yes",
        "-y",
    ...
    parser.add_argument(
        ...
        help="Path to the git checkout to update. Defaults to this TraitTutor repository.",
    )
