# TraitTutor v1.2.0 Release Notes
...
Introduced a brand-new **Book Engine** (`traittutor/book/`) that compiles user inputs — chat history, notebooks, knowledge bases, and free-form intent — into structured, block-based, interactive "living books". The engine sits parallel to `ChatOrchestrator` and drives a five-stage multi-agent pipeline:
...
Backend: `POST /api/v1/book/create`, `POST /confirm-proposal`, `POST /confirm-spine`, `POST /compile-page`, `GET /list`, `GET /{book_id}`, `DELETE /{book_id}`, plus `WS /ws/{book_id}`. CLI: `traittutor book create`, `traittutor book list`, `traittutor book show`, `traittutor book delete`.

### Legacy Guided Learning Removed
The `traittutor/agents/guide/` module (guide manager, 4 agents, 8 prompt YAMLs) and the entire `/guide` web UI (components, hooks, types, API router — ~5,300 lines) have been removed. The Book Engine supersedes Guided Learning with a richer, more extensible architecture.
...
Moved `traittutor/agents/co_writer/` to `traittutor/co_writer/` (top-level service module) to reflect its standalone nature alongside `traittutor/book/`.
...
**Full Changelog**: https://github.com/<your-org>/TraitTutor/compare/v1.1.2...v1.2.0
