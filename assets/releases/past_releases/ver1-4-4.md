# TraitTutor v1.4.4 Release Notes
...
TraitTutor skills share their format with the Agent-Skills ecosystem
(`SKILL.md` + support files), so external registries are now a native
source for the skill library:

```bash
traittutor skill search "git release notes"        # semantic search on the hub
traittutor skill install clawhub:gh-release-notes  # verify → fetch → register
traittutor skill install some-skill@1.2.0          # hub defaults to clawhub; @ pins a version
traittutor skill list                              # local skills with hub provenance
```
...
- Drop-in from v1.4.3: `pip install -U traittutor`; Docker users pull
  `ghcr.io/<your-org>/traittutor:latest`. No migrations.
- `traittutor skill install` talks to clawhub.ai over HTTPS; air-gapped
  deployments can point a mirror or a fetch CLI at it via
  `settings/skill_hubs.json`.

**Full Changelog**: https://github.com/<your-org>/TraitTutor/compare/v1.4.3...v1.4.4
