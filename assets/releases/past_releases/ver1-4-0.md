# TraitTutor v1.4.0 Release Notes
...
isolation hardening, and the `traittutor start` / `traittutor init` launcher)
...
- **`build_provider_extra_kwargs`** in `traittutor/core/agentic/client.py`
...
The `traittutor chat` REPL surface was tightened so it matches the public
docs at [traittutor.info](https://traittutor.info).

- `traittutor chat --config key=value` and `--config-json '<json>'` seed
...
- `traittutor plugin info <name>` now also returns `cli_aliases` and the
...
- `.gitignore` covers `.playwright-cli/`; stale `.playwright-cli/`
  captures, `.env.example_CN`, and `TraitTutor.code-workspace` removed.
...
- New `tests/cli/test_docs_contract.py` keeps the public site docs in
  sync with the CLI surface (links + `traittutor …` examples).
- `tests/scripts/test_docker_compose.py` + `web/tests/api-resolve-base.test.ts`
  cover the new placeholder detection.

## Upgrade Notes
- **From v1.4.0-beta:** drop-in. PyPI installs with `pip install -U traittutor`
...
**Full Changelog**: https://github.com/<your-org>/TraitTutor/compare/v1.4.0-beta...v1.4.0
