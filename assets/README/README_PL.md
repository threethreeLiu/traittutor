...
# TraitTutor: Dożywotnie Spersonalizowane Korepetycje
...
[Funkcje](#-główne-funkcje) · [Zacznij](#-pierwsze-kroki) · [Eksploruj](#-eksploracja-traittutor) · [CLI](#️-traittutor-cli--interfejs-natywny-dla-agentów) · [Ekosystem](#-ekosystem--eduhub-i-społeczność-umiejętności) · [Społeczność](#-społeczność)

</div>

---

> 🤝 **Zapraszamy do wszelkich form współpracy!** Głosuj na elementy planu działania lub proponuj nowe w [`Roadmap`](https://github.com/<your-org>/TraitTutor/issues/498), a szczegóły dotyczące strategii gałęzi, standardów kodowania i sposobu rozpoczęcia pracy znajdziesz w naszym [Przewodniku dla współtwórców](../../CONTRIBUTING.md).

### 📰 Aktualności

- **2026-05-22** 🌐 Oficjalna strona dokumentacji dostępna na [**traittutor.info**](https://traittutor.info/) — przewodniki, odniesienia i wycieczki po możliwościach w jednym miejscu.
...
- **2026-04-10** 📄 Nasz artykuł jest już dostępny na arXiv — przeczytaj [preprint](https://arxiv.org/abs/2604.26962), aby dowiedzieć się więcej o projekcie i pomysłach stojących za TraitTutor.
- **2026-02-06** 🚀 10 tys. gwiazdek w zaledwie 39 dni! Ogromne podziękowania dla naszej niesamowitej społeczności.
- **2026-01-01** 🎊 Szczęśliwego Nowego Roku! Dołącz do naszego [Discorda](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/<your-org>/TraitTutor/issues/78) lub [Dyskusji](https://github.com/<your-org>/TraitTutor/discussions) — razem kształtujmy przyszłość TraitTutor.
- **2025-12-29** 🎓 TraitTutor jest oficjalnie wydany!

## ✨ Główne funkcje

TraitTutor to natywne dla agentów środowisko nauki, które łączy korepetycje, rozwiązywanie problemów, generowanie quizów, badania, wizualizacje i ćwiczenia opanowania wiedzy w jednym rozszerzalnym systemie.
...
TraitTutor oferuje cztery ścieżki instalacji. Wszystkie współdzielą jeden układ obszaru roboczego: ustawienia żyją w `data/user/settings/` pod katalogiem, z którego uruchamiasz (lub pod `TRAITTUTOR_HOME` / `traittutor start --home` jeśli ustawisz je jawnie). Dla pełnej aplikacji zalecany przepływ to **wybierz katalog obszaru roboczego → zainstaluj → `traittutor init` → `traittutor start`**.

<details>
<summary><b>Opcja 1 — Instalacja z PyPI</b> · pełna lokalna aplikacja Web + CLI, bez potrzeby klonowania</summary>

Pełna lokalna aplikacja Web + CLI, bez potrzeby klonowania. Wymaga **Python 3.11–3.13** i środowiska uruchomieniowego **Node.js 20+** na PATH (spakowany serwer standalone Next.js jest uruchamiany przez `traittutor start`).

```bash
mkdir -p my-traittutor && cd my-traittutor
pip install -U traittutor
traittutor init     # pyta o porty + dostawcę LLM + opcjonalne osadzanie
traittutor start    # uruchamia backend + frontend; utrzymuj terminal otwarty
```

`traittutor init` pyta o port backendu (domyślnie `8001`), port frontendu (domyślnie `3782`), dostawcę LLM / bazowy URL / klucz API / model i opcjonalnego dostawcę osadzania dla Bazy wiedzy / RAG.

Po `traittutor start` otwórz adres URL frontendu wydrukowany w terminalu — domyślnie [http://127.0.0.1:3782](http://127.0.0.1:3782). Naciśnij `Ctrl+C` w tym terminalu, aby zatrzymać backend i frontend. Pominięcie `traittutor init` jest dobre dla szybkiego testu; aplikacja uruchamia się z domyślnymi portami i pustymi ustawieniami modelu, skonfiguruj je później w **Settings → Models**.
...
**Zablokowany serwer deweloperski:** jeśli `traittutor start` zgłasza istniejący frontend, który nie odpowiada, zatrzymaj PID, który wydrukuje. Jeśli żaden proces Next.js nie jest uruchomiony, pliki blokady są przestarzałe — usuń je i spróbuj ponownie:
...
- `ghcr.io/<your-org>/traittutor:latest` — stabilne wydanie
- `ghcr.io/<your-org>/traittutor:pre` — wersja wstępna, gdy dostępna
...
Otwórz [http://127.0.0.1:3782](http://127.0.0.1:3782). Kontener tworzy `/app/data/user/settings/*.json` przy pierwszym uruchomieniu; skonfiguruj dostawców modeli ze strony Web Settings. Konfiguracja, klucze API, dzienniki, pliki obszaru roboczego, pamięć i bazy wiedzy są przechowywane w woluminie `traittutor-data`.
...
- **Odłączony:** dodaj `-d`, następnie `docker logs -f traittutor`, aby śledzić, `docker stop traittutor`, aby zatrzymać, `docker rm traittutor` przed ponownym użyciem nazwy. Wolumin `traittutor-data` przechowuje ustawienia i obszar roboczy między restartami.
...
`traittutor init --cli` współdzieli ten sam układ `data/user/settings/` co pełna aplikacja, ale pomija monity o porty backendu/frontendu i domyślnie wyłącza osadzanie (wybierz `Yes` jeśli planujesz używać `traittutor kb …` lub narzędzi RAG). Nadal zapisuje kompletny układ środowiska uruchomieniowego (`system.json`, `auth.json`, `integrations.json`, `model_catalog.json`, `main.yaml`, `agents.yaml`) i nadal pyta o aktywnego dostawcę LLM i model.

<details>
<summary><b>Typowe polecenia</b></summary>

```bash
traittutor chat                                          # interaktywny REPL
...
Lokalna instalacja `traittutor-cli` nie zawiera zasobów webowych ani zależności serwera. Zachowaj kod źródłowy — instalacja edytowalna wskazuje na niego. Aby dodać aplikację Web później, zainstaluj pakiet PyPI (Opcja 1) i uruchom `traittutor init` + `traittutor start` z tego samego obszaru roboczego.
...
- **docker-compose:** zamiast tego kierowany do utwardzonego, o najmniejszych uprawnieniach **sidecar runner** (`Dockerfile.runner`) przez `TRAITTUTOR_SANDBOX_RUNNER_URL` — najsilniejsza postawa, automatycznie preferowana gdy jest obecna.

Piaskownica podprocesu jest kontrolowana przez ustawienie `sandbox_allow_subprocess` w `data/user/settings/system.json` (domyślnie `true`). Uruchamianie kodu generowanego przez model na twoim hoście to prawdziwa decyzja dotycząca zaufania — ustaw na `false` (lub eksportuj `TRAITTUTOR_SANDBOX_ALLOW_SUBPROCESS=0`), aby wyłączyć wykonanie po stronie hosta kosztem tego, że umiejętności biurowe nie będą mogły produkować plików.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="Strona główna TraitTutor — obszar roboczy Chat z każdą powierzchnią w pasku bocznym" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="Architektura systemu TraitTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="Obszar roboczy czatu TraitTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="Pętla agenta czatu TraitTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="Obszar roboczy Partners TraitTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="Architektura Partners TraitTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="Obszar roboczy My Agents TraitTutor" width="900">
</div>

My Agents zamienia innych agentów w kontekst dla TraitTutor i wykonuje dwie odrębne rzeczy. **Połącz żywego agenta** — CLI Claude Code, Codex, Gemini, Kimi, opencode lub MiMo Code na twoim komputerze, lub jednego z twoich Partners — i konsultuj go z wnętrza tury czatu: TraitTutor faktycznie *uruchamia* drugiego agenta i strumieniuje jego pracę do panelu Activity przez narzędzie `consult_subagent`. Wybierz go chipem Agent (lub wpisz `@`) i ustaw ile rund może trwać konsultacja.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="Obszar roboczy Co-Writer TraitTutor" width="900">
</div>
...
Jego wyróżniającym pomysłem jest **precyzyjna edycja**: zaznacz fragment i poproś TraitTutor o przepisanie, rozszerzenie lub skrócenie. Agent edycji może ugruntować zmianę w bazie wiedzy lub dowodach webowych, zachowuje ślad swoich wywołań narzędzi i pokazuje każdą zmianę jako diff akceptuj/odrzuć — więc nic nie ląduje dopóki tego nie zatwierdzisz.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="Biblioteka książek TraitTutor" width="900">
</div>
...
Każdy rozdział kompiluje się do typowanych bloków — tekst, callouts, quizy, fiszki, osie czasu, kod, figury, interaktywny HTML, animacje, grafy konceptów, dogłębne analizy i notatki użytkownika — a każda strona ma swój własny Page Chat. Bloki są edytowalne: wstawiaj, przenoś, regeneruj lub zmieniaj typ bloku bez przepisywania rozdziału. Polecenia konserwacyjne takie jak `traittutor book health` i `traittutor book refresh-fingerprints` pomagają wykryć kiedy wiedza źródłowa odbiega od skompilowanych stron.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="Knowledge Center TraitTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="Centrum Learning Space TraitTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="Przegląd Memory TraitTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="Graf pamięci TraitTutor" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="Centrum ustawień TraitTutor" width="900">
</div>
...
Uwierzytelnianie jest **domyślnie wyłączone** — TraitTutor działa jednoosobowo. Włącz je a jedno drzewo `data/` obsługuje obszar roboczy administratora, izolowane obszary robocze per-użytkownik i obszary robocze partnerów obok siebie:
...
**Włącz:** włącz uwierzytelnianie w `data/user/settings/auth.json`, uruchom ponownie `traittutor start`, zarejestruj pierwszego administratora pod `/register`, następnie dodaj użytkowników z `/admin/users` i przypisz modele, KB, umiejętności, Partners, politykę narzędzi/MCP i dostęp do wykonania kodu przez uprawnienia.
...
## ⌨️ TraitTutor CLI — Interfejs natywny dla agentów

Jeden plik binarny `traittutor`, dwa sposoby wejścia: interaktywny **REPL** dla osób żyjących w terminalu i strukturalny **JSON** dla innych agentów które prowadzą TraitTutor jako narzędzie. Te same możliwości, narzędzia i bazy wiedzy w obu przypadkach.

<details>
<summary><b>Prowadź sam</b></summary>

`traittutor chat` otwiera interaktywny REPL; `traittutor run <capability> "<message>"` uruchamia jedną turę i wychodzi. Oba używają tych samych flag `--capability`, `--tool`, `--kb` i `--config`.
...
TraitTutor jest zbudowany aby być *obsługiwany przez innego agenta*. Dodaj `--format json` do dowolnego `run` a każda tura strumieniuje **NDJSON — jedno zdarzenie na linię** (`content`, `tool_call`, `tool_result`, `done`, …), każda linia oznaczona swoim `session_id`. Uruchomienia są bezpieczne bez TTY: pauza `ask_user` bez TTY automatycznie rozwiązuje się pustą odpowiedzią zamiast zawieszać.
...
| Polecenie | Opis |
|:---|:---|
| `traittutor init` | Utwórz lub zaktualizuj `data/user/settings` dla bieżącego obszaru roboczego |
| `traittutor start [--home PATH]` | Uruchom backend + frontend razem |
| `traittutor serve [--port PORT]` | Uruchom tylko backend FastAPI |
| `traittutor run <capability> <message>` | Uruchom jedną turę możliwości (`chat`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`); dodaj `--format json` dla wyjścia NDJSON |
| `traittutor chat` | Interaktywny REPL z kontrolkami możliwości, narzędzia, KB, notatnika i historii |
| `traittutor partner list/create/start/stop` | Zarządzaj partnerami połączonymi przez IM |
| `traittutor kb list/info/create/add/search/set-default/delete` | Zarządzaj bazami wiedzy LlamaIndex |
| `traittutor skill search/install/list/remove/login/logout/publish/update` | Zarządzaj umiejętnościami, instaluj z hubów i publikuj własne (`eduhub:<slug>` domyślnie, patrz Ekosystem) |
| `traittutor memory show/clear` | Inspekcjonuj dokumenty pamięci L2/L3 lub wyczyść pamięć L1/wszystko |
| `traittutor session list/show/open/rename/delete` | Zarządzaj współdzielonymi sesjami |
| `traittutor notebook list/create/show/add-md/replace-md/remove-record` | Zarządzaj notatnikami z plików Markdown |
| `traittutor book list/health/refresh-fingerprints` | Inspekcjonuj książki i odświeżaj odciski źródeł |
| `traittutor plugin list/info` | Inspekcjonuj zarejestrowane narzędzia i możliwości |
| `traittutor config show` | Wydrukuj podsumowanie konfiguracji |
| `traittutor provider login <provider>` | Uwierzytelnianie dostawcy (`openai-codex` logowanie OAuth; `github-copilot` weryfikuje istniejącą sesję auth Copilot) |
...
Pakiet tylko CLI żyje w `packaging/traittutor-cli`. W tym kodzie źródłowym zainstaluj go ze źródła:
...
Umiejętności TraitTutor używają otwartego formatu **Agent-Skills** — folder z podręcznikiem `SKILL.md` (frontmatter YAML + Markdown) i opcjonalnymi plikami referencyjnymi. Nic w tym nie jest specyficzne dla TraitTutor, więc każdy rejestr który mówi w tym formacie staje się źródłem dla twojej biblioteki. TraitTutor jest dostarczany z **[EduHub](https://eduhub.traittutor.info/)** — naszym własnym rejestrem umiejętności skupionym na edukacji — podłączonym jako domyślny hub.

<details>
<summary><b>EduHub — Ekosystem umiejętności TraitTutor</b></summary>

[**EduHub**](https://eduhub.traittutor.info/) to hub społecznościowy który TraitTutor uruchomił do dzielenia się umiejętnościami agentów zorientowanymi na nauczanie — tutorzy sokratejscy, kreatory fiszek, informacje zwrotne na eseje, plany egzaminów, objaśniacze konceptów i więcej. Jest wbudowany w TraitTutor, więc nie ma nic do konfigurowania: gołe slug lub prefiks `eduhub:` rozwiązuje do niego.
...
```bash
traittutor skill search "socratic tutor"               # wyszukaj EduHub (domyślny hub)
traittutor skill install socratic-tutor                # pobierz → weryfikuj → zarejestruj
traittutor skill install eduhub:socratic-tutor@1.2.0   # przypnij hub i wersję
traittutor skill list                                  # lokalne umiejętności z proweniencją hubu
```

**Opublikuj własną** — spakuj `SKILL.md` i podziel się ze społecznością:

```bash
traittutor skill login                                 # logowanie przeglądarki do EduHub
traittutor skill publish ./my-skill                    # interaktywne: wybierz ścieżkę + tagi, następnie prześlij
traittutor skill update                                # wycofaj lub wydaj nową wersję
```

EduHub jest również samodzielnym, kompatybilnym z ClawHub rejestrem, więc agenty które nie są TraitTutorem (Claude Code, Codex, …) mogą używać go bezpośrednio przez CLI `eduhub` — `npx eduhub install socratic-tutor`.
...
- frontmatter jest normalizowany do schematu TraitTutor a `always:` jest **usuwane**, więc pobrana umiejętność nigdy nie może wymusić się do każdego systemowego promptu;
...
Ponieważ TraitTutor mówi otwartym formatem Agent-Skills, **[ClawHub](https://clawhub.ai/)** działa również jako pierwszorzędne źródło — jest wbudowany obok EduHub. Wybierz go z prefiksem hubu:
...
TraitTutor to projekt open-source, który iteruje w **w pełni open-source formie**, budowany razem ze społecznością. Do tej pory **NIE** mamy żadnych płatnych produktów online w jakiejkolwiek formie.

### 🙏 Podziękowania

Jesteśmy głęboko wdzięczni **społeczności open-source**: wasze gwiazdki, zgłoszenia, pull requesty i dyskusje kształtują TraitTutor każdego dnia.

TraitTutor stoi również na ramionach wybitnych projektów open-source które dostarczyły nam zarówno narzędzia jak i inspirację:
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | Agentowy CLI kodowania który zainspirował pętlę agenta TraitTutor |
...
Chcemy aby TraitTutor stale iterował i się rozwijał — a ostatecznie stał się prezentem który oddajemy społeczności open-source. Nasz [**plan działania**](https://github.com/<your-org>/TraitTutor/issues/498) jest aktualizowany na bieżąco; głosuj na elementy tam lub proponuj nowe. Jeśli chcesz współtworzyć, zapoznaj się z [**Przewodnikiem dla współtwórców**](../../CONTRIBUTING.md) po strategię gałęzi, standardy kodowania i sposób rozpoczęcia.

<div align="center">

Mamy nadzieję, że TraitTutor stanie się prezentem dla społeczności. 🎁

<a href="https://github.com/<your-org>/TraitTutor/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=<your-org>/TraitTutor&max=999" alt="Współtwórcy" />
</a>
...
<a href="https://www.star-history.com/#<your-org>/TraitTutor&type=timeline&legend=top-left">
  <picture>
    ...
    <img alt="Wykres historii gwiazdek" src="https://api.star-history.com/svg?repos=<your-org>/TraitTutor&type=timeline&legend=top-left" />
  </picture>
</a>
...
<p align="center">
 <a href="https://www.star-history.com/<your-org>/traittutor">
  <picture>
   ...
   <img alt="Ranking historii gwiazdek" src="https://api.star-history.com/badge?repo=<your-org>/TraitTutor" />
  </picture>
 </a>
</p>
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=<your-org>.TraitTutor&style=for-the-badge&color=00d4ff" alt="Odwiedziny">
</p>
