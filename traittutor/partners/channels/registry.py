...
def discover_channel_names() -> list[str]:
    ...
    import traittutor.partners.channels as pkg
...
def load_channel_class(module_name: str) -> type[BaseChannel]:
    ...
    from traittutor.partners.channels.base import BaseChannel as _Base

    mod = importlib.import_module(f"traittutor.partners.channels.{module_name}")
    for attr in dir(mod):
        obj = getattr(mod, attr)
        if isinstance(obj, type) and issubclass(obj, _Base) and obj is not _Base:
            return obj
    raise ImportError(f"No BaseChannel subclass in traittutor.partners.channels.{module_name}")
...
def discover_plugins() -> dict[str, type[BaseChannel]]:
    ...
    for ep in entry_points(group="traittutor.partners.channels"):
        try:
            cls = ep.load()
            plugins[ep.name] = cls
