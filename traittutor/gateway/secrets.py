...
def _fernet() -> Fernet:
    ...
    if not raw:
        raise GatewaySecretError("TRAITTUTOR_GATEWAY_MASTER_KEY is required for hosted provider secrets.")
    try:
...
def decrypt_secret(token: str) -> str:
    try:
        ...
        raise GatewaySecretError("Gateway secret cannot be decrypted with the configured master key.") from exc
