...
* **Fallback** (:meth:`_single_pass`) — for providers without native tool
  ...
  single pass, modelled on :class:`~traittutor.agents.notebook.analysis_agent`.
"""
...
from traittutor.core.agentic import dispatch_tool_calls
...
from traittutor.services.llm import clean_thinking_tags, get_llm_config
...
class ContextExplorer:
        ...
        async with stream.stage(EXPLORE_STAGE, source=EXPLORE_SOURCE, metadata=stage_meta):
            ...
            for round_idx in range(MAX_LOOP_ROUNDS):
                ...
                result = await self._call_llm(
                    messages, tool_schemas if not is_last else None, chunk_meta, stream
                )
    ...
    async def _call_llm(
        ...
        request = GatewayRequest(
            prompt="",
            system_prompt="",
            purpose="agent:explore-context:loop",
            messages=tuple(GatewayMessage.from_mapping(message) for message in messages),
            tools=tuple(GatewayTool.from_mapping(schema) for schema in (tool_schemas or [])),
        ...
        async for event in get_gateway().stream(request):
            if event.type == "reasoning" and event.text:
                ...
                arguments = json.dumps(dict(event.tool_call.arguments), ensure_ascii=False)
                output_chars += len(event.tool_call.name) + len(arguments)
                tool_calls.append(
                    {
                ...
                if event.finish_reason == "error":
                    raise RuntimeError("Context exploration Gateway stream failed")
                saw_terminal = True
        if not saw_terminal:
            raise RuntimeError("Context exploration Gateway stream ended without a receipt")
        return _CallResult(
        ...
        async with stream.stage(EXPLORE_STAGE, source=EXPLORE_SOURCE, metadata=stage_meta):
            ...
            try:
                request = GatewayRequest(
                    ...
                    purpose="agent:explore-context:single-pass",
                    messages=(
                ...
                async for chunk in get_gateway().stream_text(request, include_reasoning=True):
                    if not chunk:
                        continue
                    chunks.append(chunk)
                    await stream.thinking(
