...
(resolved by :mod:`traittutor.capabilities.subagent.binding`). As a
...
from traittutor.capabilities.protocol import KnowledgeCapability, PromptBlock
from traittutor.capabilities.subagent.binding import connection_for_turn, subagent_refs
from traittutor.capabilities.subagent.tools import SUBAGENT_TOOL_NAMES
...
class SubagentCapability(KnowledgeCapability):
    ...
    def augment_kwargs(
        ...
        if conn is None:
            return kwargs
        from traittutor.services.subagent import load_subagent_settings
        from traittutor.services.subagent.sessions import get_session, session_key
        ...
        # session the user/TraitTutor built up earlier (and the sidebar shares).
...
_DEFAULT_CONSULT_INSTRUCTION = (
    "You are being consulted programmatically by TraitTutor on the user's behalf, "
...
def _resolve_budget(context: UnifiedContext) -> int:
    ...
    """
    from traittutor.services.subagent import load_subagent_settings
    from traittutor.services.subagent.config import CONSULT_BUDGET_MAX, CONSULT_BUDGET_MIN
...
def _system_text(language: str, name: str, budget: int, kind: str = "") -> str:
    from traittutor.services.subagent import PARTNER_BACKEND_KIND
