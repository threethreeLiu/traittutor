...
# ``CONFIGURABLE_BUILTIN_TOOL_NAMES`` so settings and runtime composition
# cannot drift apart.
...
def default_optional_tools(excluded: Iterable[str] = ()) -> list[str]:
    ...
    Sourced from :mod:`traittutor.tools.builtin` so the /settings/tools UI
...
def compose_enabled_tools(
    ...
    *adds* its owned tools — it never curates or suppresses the reused
    ...
    :class:`~traittutor.capabilities.protocol.KnowledgeCapability`): the turn
    ...
    exactly as before. A set restricts which built-ins may mount for
    server-authored modes.
    ...
    if exclusive:
        ...
        return _ordered_unique([*owned, *extra, "ask_user"])
    ...
    for always_on in ("write_memory", "web_fetch", "github", "ask_user"):
...
def user_has_memory() -> bool:
    """Whether the active user has recallable canonical memory."""
    try:
        from traittutor.memory.runtime import get_current_memory_store

        return any(
            item.sensitivity != "sensitive"
            for item in get_current_memory_store().list_items(status="active")
        )
    except Exception:
        return False
