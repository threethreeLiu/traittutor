...
from traittutor.core.trace import merge_trace_metadata
from traittutor.i18n import StatusI18n
...
class VisualizeCapability(BaseCapability):
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        from traittutor.agents.visualize.models import ReviewResult
        from traittutor.agents.visualize.pipeline import VisualizePipeline
        from traittutor.agents.visualize.utils import (
            build_fallback_html,
        ...
        *,
        ...
        if importlib.util.find_spec("manim") is None:
            raise RuntimeError(
                ...
                "Install with `pip install 'traittutor[math-animator]'` "
                "or `pip install -r requirements/math-animator.txt`."
            )

        from traittutor.agents.math_animator.pipeline import MathAnimatorPipeline
        from traittutor.agents.math_animator.request_config import MathAnimatorRequestConfig
        from traittutor.core.trace import build_trace_metadata, new_call_id
        from traittutor.services.llm.config import get_llm_config
