...
to :mod:`traittutor.core.agentic` and the shared tool-composition policy.
"""
...
from traittutor.core.agentic.labels import find_inline_labels
from traittutor.core.agentic.tool_dispatch import MAX_PARALLEL_TOOL_CALLS
from traittutor.core.agentic.usage import record_streamed_usage
...
class QuestionPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        exploring_value = self.runtime_config.get("exploring")
        exploring_cfg: dict[str, Any] = exploring_value if isinstance(exploring_value, dict) else {}
        ...
        summarizer_value = exploring_cfg.get("tool_summarizer")
        summarizer_cfg: dict[str, Any] = (
            summarizer_value if isinstance(summarizer_value, dict) else {}
        )
    ...
    async def _summarize_tool_result(
        ...
        *,
        ...
        meta = build_trace_metadata(
            ...
            label=self._t("labels.reflecting", default="TraitTutor Reflecting..."),
        ...
        await stream.progress(
            self._t("labels.reflecting", default="TraitTutor Reflecting..."),
            source=SOURCE,
            stage=STAGE_EXPLORING,
    ...
    def _use_native_tools(self, context: UnifiedContext) -> bool:
        ...
        return bool(self._resolved_tools(context)) and gateway_supports_tools(
            self.binding, self.model
        )
    ...
    # LLM call helpers
    # ------------------------------------------------------------------
    def _build_client_for_context(self, context: UnifiedContext) -> GatewayAgenticClient:
        """Build the only provider boundary allowed for QuestionPipeline."""
        ...
        if not owner_id or not isinstance(cancellation_event, asyncio.Event):
            raise RuntimeError(
                "Gateway Deep Question requires a server-owned turn and cancellation scope."
            )
        return GatewayAgenticClient(
            owner_id=owner_id,
            cancellation_event=cancellation_event,
            llm_config=self.llm_config,
            reasoning_effort=self.reasoning_effort,
            purpose_prefix="question",
            surface_name="Deep Question",
        )
...
class _BaseLoopHost:
    ...
    async def before_iteration(
        ...
        *,
        ...
        """Question loops need no per-iteration mutation by default."""
        return None
