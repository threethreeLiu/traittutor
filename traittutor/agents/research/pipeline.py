...
from traittutor.agents.research.data_structures import (
    DynamicTopicQueue,
    ToolTrace,
    TopicBlock,
    TopicStatus,
...
from traittutor.agents.research.utils.citation_manager import CitationManager
...
from traittutor.services.llm import get_llm_config, prepare_multimodal_messages
...
from traittutor.services.sandbox import exec_capability_available
...
class ResearchPipeline:
    ...
    LLM call goes through :mod:`traittutor.core.agentic` primitives. The
    ...
    def __init__(
        ...
        *,
        ...
        researching_value = self.runtime_config.get("researching")
        researching: dict[str, Any] = (
            researching_value if isinstance(researching_value, dict) else {}
        )
        planning_value = self.runtime_config.get("planning")
        planning: dict[str, Any] = planning_value if isinstance(planning_value, dict) else {}
        reporting_value = self.runtime_config.get("reporting")
        reporting: dict[str, Any] = reporting_value if isinstance(reporting_value, dict) else {}
        queue_value = self.runtime_config.get("queue")
        queue_cfg: dict[str, Any] = queue_value if isinstance(queue_value, dict) else {}
        ...
        rephrase_value = planning.get("rephrase")
        rephrase_cfg: dict[str, Any] = rephrase_value if isinstance(rephrase_value, dict) else {}
        self.rephrase_enabled = bool(rephrase_cfg.get("enabled", True))
        ...
        # All phases share the typed Gateway route selected for this runtime.
    ...
    async def run(
        ...
        *,
        ...
        client = self._build_client_for_context(context)
    ...
    async def _rephrase(
        ...
        *,
        ...
        tool_schemas = (
            ...
            if gateway_supports_tools(self.binding, self.model)
            else None
        )
    ...
    def _use_native_block_tools(self, tool_names: list[str] | None = None) -> bool:
        names = self._block_tool_names() if tool_names is None else tool_names
        return bool(names) and gateway_supports_tools(self.binding, self.model)
    ...
    # LLM call helpers
    # ------------------------------------------------------------------
    def _build_client_for_context(self, context: UnifiedContext) -> Any:
        """Return the server-scoped typed Gateway adapter.

        The path is intentionally fail-closed: all possible native
        tool phases require a provider with typed function-tool support, and
        the authenticated turn runtime must supply the private owner/cancel
        scope.  Falling back here would permit a legacy provider request after
        a previous typed tool round has changed server state.
        """

        needs_tools = bool(self._block_tool_names()) or (
            self.rephrase_enabled and self._tool_in_registry("ask_user")
        )
        if needs_tools and not gateway_supports_tools(self.binding, self.model):
            raise RuntimeError(
                "Gateway Deep Research requires a provider with typed function-tool support."
            )

        owner_id = str(context.metadata.get("gateway_owner_id") or "").strip()
        cancellation_event = context.metadata.get("gateway_cancellation_event")
        if not owner_id or not isinstance(cancellation_event, asyncio.Event):
            raise RuntimeError(
                "Gateway Deep Research requires a server-owned turn and cancellation scope."
            )
    ...
    * :meth:`force_finalize` — drives the per-block force-finish
    ...
    async def before_iteration(
        ...
        *,
        ...
        """Block loops currently require no per-iteration message mutation."""
        return None
...
class _RephraseLoopHost:
    ...
    async def before_iteration(
        ...
        *,
        ...
        """Rephrase loops currently require no per-iteration message mutation."""
        return None
    ...
    async def resolve_pause(self, dispatch: DispatchOutcome) -> bool:
        from traittutor.agents.chat.agentic_pipeline import (
