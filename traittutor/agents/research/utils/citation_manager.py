...
class CitationManager:
    ...
    def __init__(self, research_id: str, cache_dir: Path | None = None):
        ...
        if cache_dir is None:
            cache_dir = path_service.get_task_workspace("deep_research", research_id)
        ...
        if custom_cache_dir:
            document_store = SQLiteDocumentStore(
                ...
                db_path=self.cache_dir.parent / "traittutor.sqlite3",
            )
        else:
            document_store = SQLiteDocumentStore(
                owner_id,
    ...
    def _load_citations(self):
        """Load citation information from SQLite and restore counters."""
        data = self._document_store.load(self.research_id, {})
        if not isinstance(data, dict):
            self._citations = {}
            return
        citations = data.get("citations", {})
        self._citations = citations if isinstance(citations, dict) else {}
        counters = data.get("counters", {})
        if isinstance(counters, dict) and counters:
            self._plan_counter = int(counters.get("plan_counter", 0))
            block_counters = counters.get("block_counters", {})
            self._block_counters = dict(block_counters) if isinstance(block_counters, dict) else {}
        else:
    ...
    def _save_citations(self):
        """Save citation information to SQLite."""
        data = {
        ...
        self._document_store.save(self.research_id, data)
                    ...
                    if isinstance(source_list, list):
                        for i, doc in enumerate(source_list[:5]):  # Limit to 5 sources
                            ...
                            if isinstance(doc, dict):
                                ...
                                source_info["content_preview"] = str(
