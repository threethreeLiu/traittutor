...
class DynamicTopicQueue:
    ...
    def save_to_json(self, filepath: str) -> None:
        """Persist queue state in SQLite (method name kept for API compatibility)."""
        ...
        adapter = SectionedRecordStore(
            "legacy_research_queues",
            get_current_user().id,
            schema_version=1,
            legacy_path=filepath,
        )
        record = {**self.to_dict(), "owner_id": get_current_user().id}
        with adapter.locked() as payload:
            payload["queues"] = [
                item for item in payload["queues"] if item.get("research_id") != self.research_id
            ]
            payload["queues"].append(record)
    ...
    def load_from_json(cls, filepath: str) -> "DynamicTopicQueue":
        ...
        return cls.from_dict(records[-1])
