...
from traittutor.agents._shared.tool_composition import (
    ToolMountFlags,
...
from traittutor.agents.chat.prompt_blocks import ChatPromptAssembler
from traittutor.capabilities import (
    LoopCapability,
    active_loop_capabilities,
    any_exclusive_capability_active,
)
from traittutor.core.agentic import (
    DispatchOutcome,
    UsageTracker,
    build_completion_kwargs,
    dispatch_tool_calls,
)
from traittutor.core.agentic.tool_dispatch import MAX_PARALLEL_TOOL_CALLS
from traittutor.core.context import UnifiedContext
from traittutor.core.stream_bus import StreamBus
from traittutor.core.trace import (
    build_trace_metadata,
    derive_trace_metadata,
    merge_trace_metadata,
    new_call_id,
)
from traittutor.runtime.registry.deferred_tools import (
    DeferredToolLoader,
    render_deferred_tools_manifest,
)
from traittutor.runtime.registry.tool_registry import get_tool_registry
from traittutor.services.config import get_chat_params
from traittutor.services.llm import (
    get_llm_config,
    get_token_limit_kwargs,  # noqa: F401  (re-exported for tests)
    prepare_multimodal_messages,
    supports_tools,  # noqa: F401  (re-exported for tests)
)
from traittutor.services.llm.capabilities import supports_tools as gateway_supports_tools
...
def _normalise_user_reply(raw: Any) -> tuple[str, list[dict[str, str]] | None]:
    if not isinstance(raw, dict) or not isinstance(raw.get("answers"), list):
        raise ValueError("ask_user replies require canonical answers")
    answers: list[dict[str, str]] = []
    for entry in raw["answers"]:
        if not isinstance(entry, dict):
            raise ValueError("ask_user answers must be objects")
        qid = str(entry.get("questionId") or "").strip()
        if not qid or "text" not in entry:
            raise ValueError("ask_user answers require questionId and text")
        answers.append({"questionId": qid, "text": str(entry["text"] or "")})
    if not answers:
        raise ValueError("ask_user answers cannot be empty")
    reply_text = " | ".join(answer["text"] for answer in answers if answer["text"])
    return reply_text, answers
...
class AgenticChatPipeline:
    ...
    def effective_max_rounds(self, context: UnifiedContext) -> int:
        ...
        A capability that needs guaranteed loop headroom can set
        ``context.metadata["_min_loop_rounds"]``; the loop honours the larger
        of that and the configured budget.
        """
        try:
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        ...
        if enabled_tools and not self._can_use_gateway_tool_calling():
            # Gateway intentionally does not reproduce the legacy DSML
            # text-parser fallback.  A provider that cannot accept the typed
            # function schema must fail clearly before any tool prompt or
            # side effect is produced, rather than silently interpreting
            # model prose as a privileged tool invocation.
            raise RuntimeError(
                "Gateway agentic chat requires a provider with typed function-tool support."
            )
    ...
    async def _prepare_deferred_tools(self, context: UnifiedContext) -> None:
        ...
        try:
            from traittutor.services.mcp import get_mcp_manager, load_loaded_tools

            await get_mcp_manager().ensure_started()
            # Non-admin users fail closed until an administrator explicitly
            # grants MCP tool names; administrators remain unrestricted.
            from traittutor.multi_user.tool_access import allowed_mcp_tools
            ...
            if self._pageindex_docs:
                from traittutor.services.mcp.pageindex_server import PAGEINDEX_SERVER_NAME
    ...
    def _pageindex_doc_maps(self, context: UnifiedContext) -> dict[str, dict[str, str]]:
        ...
        for kb in self._selected_kbs(context):
            try:
                from traittutor.multi_user.knowledge_access import resolve_kb
                from traittutor.services.rag.factory import PAGEINDEX_PROVIDER
                from traittutor.services.rag.pipelines.pageindex.pipeline import PageIndexPipeline
                from traittutor.services.rag.provider_binding import resolve_bound_provider
    ...
    async def _exec_allowed(self, context: UnifiedContext) -> bool:
        try:
            from traittutor.services.sandbox import IsolationLevel, get_sandbox_service
            ...
            if level is IsolationLevel.SYSTEM:
                ...
                from traittutor.multi_user.tool_access import exec_override
            ...
            if level is IsolationLevel.APPLICATION:
                try:
                    ...
                    # Single-user local runtime: APPLICATION isolation remains
                    # an explicit local-development opt-in.
                    return True
            return False
        except Exception:
    ...
    def _capability_system_blocks(self, context: UnifiedContext):
        ...
        for cap in self._active_loop_capabilities(context):
            ...
            if block is not None:
                blocks.append(block)
        # Capability-injected extras (e.g. learning-support read-only canvas
        # and tutor-expression contracts) are staged in metadata by
        # ``ChatCapability`` before the pipeline runs. Merge them here so they
        # sit in the same prompt region as other capability playbooks.
        extra = context.metadata.get("_extra_capability_blocks") if context.metadata else None
        if extra:
            blocks.extend(extra)
        return blocks
    ...
    async def _execute_tool_call(
        ...
        *,
        ...
        from traittutor.core.agentic import execute_tool_call
    ...
    def _augment_tool_kwargs(
        ...
        if tool_name == "rag":
            ...
            from traittutor.services.sandbox import Mount
    ...
    def _estimate_messages_tokens(messages: list[dict[str, Any]]) -> int:
        from traittutor.services.session.context_builder import count_tokens
    ...
    def _can_use_native_tool_calling(self) -> bool:
        return self._can_use_gateway_tool_calling()

    def _can_use_gateway_tool_calling(self) -> bool:
        """Gateway tool loops require an explicitly tool-capable provider.

        The legacy client has a DSML text parser for deployments which emit
        pseudo tool calls in prose.  That representation cannot carry the
        validated Gateway replay contract, so the opt-in path must reject it
        instead of silently executing parser-derived arguments.
        """

        return gateway_supports_tools(self.binding, self.model)
