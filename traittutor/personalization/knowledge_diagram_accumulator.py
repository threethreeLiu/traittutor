...
def _repository() -> LearningKnowledgeGraphRepository:
    return LearningKnowledgeGraphRepository(
        get_path_service().get_workspace_dir() / "learner" / "knowledge-graph.sqlite3"
    )
...
def _graph_from_payload(payload: Mapping[str, Any], *, source_ref: str) -> LearningKnowledgeGraph | None:
    ...
    if isinstance(raw_edges, list):
        for raw_edge in raw_edges[:180]:
            ...
            if not source or not target or source == target:
                continue
            relation = _EDGE_RELATION_MAP.get(str(raw_edge.get("relation") or "").strip(), "related_to")
            edges.append(
                KnowledgeGraphEdge(
