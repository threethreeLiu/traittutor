...
from traittutor.learning_model.parameters import UNCALIBRATED_FALLBACK_PARAMS
...
class ReflectionView(BaseModel):
    ...
    category: Literal[
        "goal", "explanation", "pacing", "feedback", "constraint", "concept", "strategy"
    ]
...
class ConceptSignal(BaseModel):
    ...
    # MEMORY / PROCEDURE / CONCEPT / DESIGN (see learning_model.decay).
    # Decay uses the type-specific half-life when present; the learning domain
    # owns the authoritative type map (LearningProgress.knowledge_types) and
    # does not populate this field yet, so absent types fall back to the
    # default half-life.
    ...
    transition_probability: float = Field(
        default=UNCALIBRATED_FALLBACK_PARAMS.transition, ge=0, le=1
    )
    guess_probability: float = Field(default=UNCALIBRATED_FALLBACK_PARAMS.guess, ge=0, le=1)
    slip_probability: float = Field(default=UNCALIBRATED_FALLBACK_PARAMS.slip, ge=0, le=1)
    observation_count: int = Field(default=0, ge=0)
    verified_observation_count: int = Field(default=0, ge=0)
    last_observation_source: str | None = None
    bkt_param_version: str = UNCALIBRATED_FALLBACK_PARAMS.version
    bkt_calibrated: bool = False
    mastery_interval: tuple[float, float] = (0.0, 1.0)

    @field_serializer("mastery_probability")
    def hide_uncalibrated_posterior(self, value: float, info: Any) -> float | None:
        """Public dumps hide pseudo-precise uncalibrated posteriors."""
        context = info.context if isinstance(info.context, dict) else {}
        if self.bkt_calibrated or context.get("include_uncalibrated_posterior"):
            return value
        return None
...
class SubjectUnderstanding(BaseModel):
    ...
    verified_mastery: float | None = Field(default=None, ge=0, le=1)
    verified_observation_count: int = Field(default=0, ge=0)
    mastery_interval: tuple[float, float] = (0.0, 1.0)
...
class LearningSignal(BaseModel):
    ...
    def reject_sensitive_payload_keys(cls, value: dict[str, Any]) -> dict[str, Any]:
        ...
        if prohibited.intersection(key.lower() for key in value):
            raise ValueError(
                "learning signals cannot contain diagnosis, ability, personality, or learning-style fields"
            )
        return value
...
class LearnerEvent(BaseModel):
    ...
    observation: (
        Literal["correct", "incorrect", "known", "unknown", "uncertain", "engaged"] | None
    ) = None
...
class TeachingStrategyPlan(TeachingAction):
    srl_support: list[Literal["goal", "monitor", "reflect", "motivation"]] = Field(
