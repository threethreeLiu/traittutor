...
def log_intent_security_event(
    *, action: str, category: str | None, session_hash: str | None
) -> None:
    """Record minimal routing-security telemetry without retaining user text."""
    user = get_current_user()
    payload: dict[str, Any] = {
        "time": datetime.now(timezone.utc).isoformat(),
        "actor_id": user.id,
        "action": f"learn_intent_{action}",
    }
    if category:
