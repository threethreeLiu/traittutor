"""TraitTutor public product policy for bounded learner-support routing.
...
def _config() -> dict[str, Any]:
    return json.loads(
        Path(__file__).with_name("generation_support.json").read_text(encoding="utf-8")
    )
...
def _action_catalog() -> dict[str, Any]:
    ...
    return json.loads(
        Path(__file__).with_name("slr_action_catalog.json").read_text(encoding="utf-8")
    )
...
def _zone(score: Any) -> str:
    ...
    return (
        "core"
        if value <= config["core_low_max"] or value >= config["core_high_min"]
        else "edge"
        if value in {config["edge_low"], config["edge_high"]}
        else "neutral"
    )
...
def _level(score: Any) -> str:
    ...
    return (
        "high_core"
        if value >= config["core_high_min"]
        else "high_edge"
        if value == config["edge_high"]
        else "neutral"
        if value in {5, 6}
        else "low_edge"
        if value == config["edge_low"]
        else "low_core"
    )
...
def build_generation_support_profile(
    scores: Mapping[str, Any], *, task_type: str = "concept_learning"
) -> dict[str, Any]:
    config = _config()
    copy = config["copy"]
    normalized = {key: float(scores.get(key, 6) or 6) for key in ("O", "C", "E", "A", "N")}
    buckets = {
        name: _bucket(normalized[key])
        for key, name in (
            ("O", "openness"),
            ("C", "conscientiousness"),
            ("E", "extraversion"),
    ...
    levels = {
        f"{name}_level": _level(normalized[key])
        for key, name in (
            ("O", "openness"),
            ("C", "conscientiousness"),
            ("E", "extraversion"),
    ...
    support = {
        ...
        if buckets["conscientiousness"] == "low" and zones["conscientiousness"] == "core"
        else 3
        if zones["conscientiousness"] == "edge"
        else 3,
        "scaffolding_need": _complex(
            buckets["neuroticism"],
            zones["neuroticism"],
            5,
            buckets["openness"],
        ...
        "reassurance_need": _complex(
            buckets["neuroticism"],
            zones["neuroticism"],
            5,
            buckets["agreeableness"],
            zones["agreeableness"],
            4,
            2,
        ),
        "activation_need": _complex(
            buckets["extraversion"],
            zones["extraversion"],
            5,
            buckets["neuroticism"],
            zones["neuroticism"],
    ...
    risk_flags = [
        ...
        for flag, condition in (
            ("ambiguity_stress", buckets["neuroticism"] == "high"),
            ("start_friction", buckets["conscientiousness"] == "low"),
            ("social_overload_risk", buckets["extraversion"] == "low"),
            ("novelty_resistance", buckets["openness"] == "low"),
        )
        if condition
    ]
    ...
    if buckets["neuroticism"] == "high" and zones["neuroticism"] == "core":
        ...
        scaffolding = (
            copy["scaffolding_depth"]
            if support["conceptual_depth_readiness"] >= 4
            else copy["scaffolding_default"]
        )
    interpretive_notes = {
    ...
    profile = {
        "trait_scores": normalized,
        "trait_buckets": {f"{key}_bucket": value for key, value in buckets.items()},
        "trait_intervention_zones": {f"{key}_zone": value for key, value in zones.items()},
        "trait_strategy_levels": levels,
        "learner_support_profile": support,
        "baseline_learning_constraints": {
            "prior_knowledge_level": "medium",
            "recommended_pacing": "moderate",
            "explanation_granularity": "medium",
        },
        "prior_knowledge_level": "medium",
        ...
        "support_recommendations": {
            ...
            "motivation_support": copy["motivation"],
            "interaction_mode": interaction,
        },
        "interpretive_notes": interpretive_notes,
        "support_profile": dict(support),
        "do_not_assume": [
            "Do not assume a fixed learner type or immutable preference.",
            "Do not assume prior knowledge changes personality-driven support needs.",
            "Do not create standalone scaffold modules for edge-zone or neutral-zone traits.",
        ],
    ...
    return {
        ...
        "source": "traittutor_public_product_policy",
        "config_version": config["version"],
        "status": "generation_support_only",
    }


def build_slr_action_support(
    scores: Mapping[str, Any], *, task_type: str = "concept_learning"
) -> dict[str, Any]:
    ...
    strengths = {
        ...
        "reflection_transfer": max(
            needs["autonomy_support_need"], needs["conceptual_depth_readiness"]
        ),
