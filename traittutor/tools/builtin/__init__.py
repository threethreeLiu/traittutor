...
class _PromptHintsMixin:
    ...
    if TYPE_CHECKING:
        # Provided by the ``BaseTool`` this mixin is always combined with.
        def get_definition(self) -> ToolDefinition: ...

    def get_prompt_hints(self, language: str = "en"):
        # ``name`` lives on the ToolDefinition, not the tool instance.
        return load_prompt_hints(self.get_definition().name, language=language)
...
class RAGTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from traittutor.tools.rag_tool import rag_search
...
class CodeExecutionTool(_PromptHintsMixin, BaseTool):
    ...
    it through :mod:`traittutor.services.sandbox`. No second LLM call, and the
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from traittutor.services.sandbox import (
            ExecRequest,
            Mount,
            ResourceLimits,
            get_sandbox_service,
        )
        from traittutor.services.sandbox.artifacts import (
...
class PaperSearchToolWrapper(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from traittutor.tools.paper_search_tool import ArxivSearchTool
...
class GeoGebraAnalysisTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from traittutor.agents.vision_solver.vision_solver_agent import VisionSolverAgent
        from traittutor.services.llm.config import get_llm_config
...
class ReadMemoryTool(_PromptHintsMixin, BaseTool):
    """Read active, non-sensitive canonical long-term memory."""
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        store = get_current_memory_store()
        # Authorized, bounded retrieval with a persisted audit trail. A bare
        # ``list_items`` dump would skip the MemoryAccessRecord contract every
        # other prompt-facing memory read honors, and would ignore the store's
        # result/token bounds (AGENTS.md: cross-domain reads must be trimmed,
        # authorized and audited).
        items = store.search(
            purpose="chat_memory_tool",
            snapshot_id=f"snap-read-memory-{_unique_run_token()}",
        )
        lines: list[str] = []
        skipped_injection = 0
        for item in items:
            if item.sensitivity == "sensitive":
                continue
            # Memory text is user-authored content, never instructions: the
            # same injection guard used for learner material applies here.
            action, _category = scan_untrusted_learning_payload(item.value)
            if action == "block":
                skipped_injection += 1
                continue
            lines.append(f"- {item.value} (id: {item.memory_id})")
        text = "\n".join(lines)
        metadata: dict[str, Any] = {"char_count": len(text), "memory_count": len(lines)}
        if skipped_injection:
            metadata["skipped_injection_blocked"] = skipped_injection
        return ToolResult(
            content=text,
            metadata=metadata,
        )
...
class WriteMemoryTool(_PromptHintsMixin, BaseTool):
    """Persist an explicit preference through the canonical memory lifecycle."""
        ...
        return ToolDefinition(
            ...
            parameters=[
                ...
                ToolParameter(
                    name="target_id",
                    type="string",
                    description="Existing memory id (form `mem_xxx`). Required for `edit`.",
                    required=False,
                ),
                ToolParameter(
                    name="reason",
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from traittutor.memory.runtime import get_current_memory_store
        from traittutor.memory.store import MemoryActivationError
        ...
        if op == "edit" and not target_id:
            return ToolResult(
                content="write_memory rejected: target_id is required for edit",
                success=False,
                metadata={"op": op},
            )
        store = get_current_memory_store()
        try:
            item, deduplicated = store.save_explicit_preference(
                value=text,
                target_memory_id=str(target_id).strip() if target_id else None,
                operation_id=f"write-memory-{_unique_run_token()}",
            )
        except (KeyError, MemoryActivationError, ValueError) as exc:
            return ToolResult(
                content=f"write_memory rejected: {exc}",
                success=False,
                metadata={"op": op},
            )
        if deduplicated:
            content = f"preference already saved (memory={item.memory_id}); skipped duplicate."
        else:
            content = f"preference {op}ed (memory={item.memory_id})."
        return ToolResult(
            content=content,
            metadata={
                "op": op,
...
class WebFetchTool(_PromptHintsMixin, BaseTool):
    ...
    ``traittutor.tools.web_fetch`` so this wrapper stays free of network
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from traittutor.tools.web_fetch import (
            DEFAULT_MAX_CHARS,
...
class ListNotebookTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from traittutor.tools.list_notebook import list_notebooks_or_records
...
class WriteNoteTool(_PromptHintsMixin, BaseTool):
    ...
    * ``edit`` — patch an existing record's title / body / summary.
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from traittutor.tools.write_note import write_note
...
class GithubTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from traittutor.tools.github_query import run_github_query
...
class AskUserTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from traittutor.tools.ask_user import build_ask_user_payload
...
class ReadSkillTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from traittutor.services.skill.service import (
...
# two stay in lockstep. Capability-owned tools (solve/obsidian)
