...
Every command runs through the :mod:`traittutor.services.sandbox` layer rather
than a raw subprocess. The pipeline mounts the turn's workspace read-write and
exposes it as the working directory; a deny-pattern guard adds defence-in-depth
on top of OS isolation.
...
# NOTE: ``traittutor.services.sandbox`` is imported lazily inside ``execute``
# (not at module top). This tool is loaded by ``traittutor.tools.builtin``,
...
# The sandbox is the real boundary; these patterns stop obviously destructive
# commands early as defence-in-depth.
