...
async def fetch_image_from_url(url: str) -> tuple[bytes, str]:
    ...
    try:
        ...
        raise ImageError(f"Failed to download image: HTTP {e.response.status_code}") from e
    except httpx.TimeoutException:
        raise ImageError(f"Image download timeout ({REQUEST_TIMEOUT}s)") from None
    except httpx.RequestError as e:
        raise ImageError(f"Failed to download image: {e!s}") from e
