...
Uses the typed Gateway for all LLM calls, supporting:
...
from traittutor.services.llm.capabilities import supports_response_format
...
def extract_questions_with_llm(
    ...
    binding = gateway_config.binding
...
"""
    ...
    request = GatewayRequest(
        ...
        purpose="agent:question:mimic-source-extraction",
        messages=(
            GatewayMessage(role="system", content=system_prompt),
            GatewayMessage(role="user", content=user_prompt),
        ),
        response_format=(
            {"type": "json_object"} if supports_response_format(binding, model) else None
        ),
    ...
    async def _complete_request() -> str:
        return (await get_gateway().complete(request)).content

    try:
        asyncio.get_running_loop()
    except RuntimeError:
        result_text = asyncio.run(_complete_request())
    else:
        import concurrent.futures

        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(asyncio.run, _complete_request())
