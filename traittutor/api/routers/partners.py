...
from traittutor.core.i18n import t
from traittutor.partners.config.paths import get_partner_media_dir
from traittutor.partners.helpers import safe_filename
from traittutor.services.partners import (
    get_partner_manager,
    slugify_partner_id,
    slugify_soul_id,
)
...
from traittutor.services.partners.workspace import (
    list_assets,
    provision_assets,
    read_soul,
    remove_asset,
...
def _validate_channels_payload(channels: dict) -> None:
    ...
    from traittutor.partners.config.schema import ChannelsConfig
...
def _validate_llm_selection_payload(
    ...
    from traittutor.services.model_selection import apply_llm_selection_to_catalog
    from traittutor.services.partners.model_runtime import normalize_partner_llm_selection
...
def _resolve_soul_content(soul: SoulSpec | None) -> tuple[str, dict[str, str]]:
    ...
    from traittutor.services.partners.workspace import DEFAULT_SOUL
...
def _load_persona_markdown(name: str) -> str:
    ...
    from traittutor.services.persona import PersonaService, get_persona_service
    ...
    def _persona_entry(
        ...
        *,
        ...
        return {
            ...
            "content": content,
            "source": source,
            "read_only": source == "admin",
        }

    personas: list[dict[str, str | bool]] = []
    seen: set[str] = set()
    try:
        ...
        if not get_current_user().is_admin:
            ...
            for info in admin_service.list_personas():
                if info.name not in seen:
                    personas.append(
                        _persona_entry(admin_service, info, source="admin")
                    )
    except Exception:
...
async def list_channel_schemas():
    ...
    from traittutor.api.routers._partners_channel_schema import all_channel_schemas
...
async def tool_options():
    ...
    """
    from traittutor.api.utils.tool_options import build_tool_options
...
async def start_partner(partner_id: str):
    ...
    # comes back in this state after a TraitTutor restart (a manual /stop clears
...
async def partner_command_palette():
    from traittutor.services.partners.commands import partner_command_palette
...
def _partner_upload_caps() -> tuple[int, int]:
    try:
        from traittutor.services.config.runtime_settings import get_chat_attachment_limits
...
async def _partner_chat_stream(
    ...
    from traittutor.core.stream import StreamEventType
...
async def partner_chat_ws(ws: WebSocket, partner_id: str):
    ...
    * ``{"type": "done"}`` / ``{"type": "error"}`` / ``{"type": "proactive"}``.
    """
    from traittutor.api.routers.auth import ws_auth_failed, ws_require_auth
