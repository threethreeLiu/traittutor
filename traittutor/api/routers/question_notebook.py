...
class UpsertEntryRequest(BaseModel):
    """Submit one learner answer to a server-held quiz item."""
...
def _public_notebook_entry(entry: dict) -> dict:
    """Expose the server-held answer only for an already submitted entry."""
    public = dict(entry)
    public["reference_answer"] = str(public.pop("correct_answer", "") or "")
    public["explanation"] = str(public.get("explanation") or "")
    return public
...
@router.post("/entries/upsert", response_model=NotebookEntryItem)
async def upsert_single_entry(payload: UpsertEntryRequest):
    from traittutor.api.routers.sessions import (
        QuizAnswerImage,
        _grade_server_held_quiz_item,
    )

    await _grade_server_held_quiz_item(
        ...
        user_answer_images=(
            [QuizAnswerImage(**image.model_dump()) for image in payload.user_answer_images]
            if payload.user_answer_images is not None
            else None
        ),
    )
    entry = await get_sqlite_session_store().find_notebook_entry(
        payload.session_id, payload.question_id, turn_id=payload.turn_id
    )
    if entry is None:
        raise HTTPException(status_code=500, detail="Upsert failed")
    return NotebookEntryItem(**_public_notebook_entry(entry))
...
async def list_entries(
    ...
    return NotebookEntryListResponse(
        items=[NotebookEntryItem(**_public_notebook_entry(item)) for item in result["items"]],
        total=result["total"],
    )


@router.get("/entries/lookup/by-question", response_model=NotebookEntryItem)
