...
from traittutor.multi_user.model_access import allowed_llm_options
...
from traittutor.services.config.runtime_settings import (
    CHAT_ATTACHMENT_CHARS_RANGE,
    CHAT_ATTACHMENT_MAX_FILE_MB_RANGE,
    CHAT_ATTACHMENT_MAX_TOTAL_MB_RANGE,
    compute_ws_max_size,
)
from traittutor.services.embedding.client import reset_embedding_client
from traittutor.services.llm.client import reset_llm_client
from traittutor.services.llm.config import clear_llm_config_cache
...
from traittutor.tools.builtin import USER_TOGGLEABLE_TOOL_NAMES
from traittutor.unified_storage import SectionedRecordStore, SQLiteDocumentStore

router = APIRouter()


def _tour_store() -> SQLiteDocumentStore:
    return SQLiteDocumentStore(
        LOCAL_ADMIN_ID,
        namespace="setup-tour",
...
DEFAULT_UI_SETTINGS = {
    # Snow is the product default for new installs.
    "theme": "snow",
    "language": "zh",
    ...
    # stale names are ignored on read.
...
class UISettings(BaseModel):
    theme: Literal["light", "dark", "snow", "teal"] = "snow"
...
class UISettingsUpdate(BaseModel):
    ...
    theme: Literal["light", "dark", "snow", "teal"] | None = None
...
class ThemeUpdate(BaseModel):
    theme: Literal["light", "dark", "snow", "teal"]
...
class DocumentParsingUpdate(BaseModel):
    ...
    Engine slices are merged into the canonical document-parsing contract.
    """
...
def load_ui_settings() -> dict[str, Any]:
    ...
    adapter = SectionedRecordStore(
        "interface_settings", owner, schema_version=1, path_service=get_path_service()
    )
    record = next(iter(adapter.snapshot()["settings"]), None)
    saved = dict(record.get("value") or {}) if record else {}
    merged = {**DEFAULT_UI_SETTINGS, **saved}
    merged["enabled_optional_tools"] = _sanitize_enabled_tools(merged.get("enabled_optional_tools"))
    return merged
...
def save_ui_settings(settings: dict[str, Any]) -> None:
    ...
    adapter.replace_all(
        {
            "schema_version": 1,
            "settings": [{"settings_id": "interface", "owner_id": owner, "value": settings}],
        }
    )
...
def _provider_choices() -> dict[str, list[dict[str, str]]]:
    ...
    from traittutor.services.config.provider_runtime import (
        EMBEDDING_PROVIDERS,
        IMAGEGEN_PROVIDERS,
        STT_PROVIDERS,
        TTS_PROVIDERS,
    ...
    from traittutor.services.provider_registry import PROVIDERS
...
def _mineru_settings_payload() -> dict[str, Any]:
    ...
    """
    from traittutor.services.parsing.engines.mineru.backend import local_cli_probe
...
def _document_parsing_payload() -> dict[str, Any]:
    ...
    from traittutor.services.parsing.engines._install import (
        installable_engines,
        model_downloadable_engines,
    )
    from traittutor.services.parsing.engines.factory import (
        get_parser,
        list_engines,
    )
    from traittutor.services.parsing.engines.mineru.backend import local_cli_probe
...
async def test_document_parsing(payload: DocumentParsingTest):
    ...
    _require_settings_admin()
    from traittutor.services.parsing.engines.factory import get_parser, is_engine_available
    ...
    if not cmd:
        return {
            ...
            "message": (
                ...
                f"(pip install traittutor[parse-{engine}]) so its CLI is on PATH."
            ),
        }
...
async def document_parsing_job_status(cursor: int = 0):
    _require_settings_admin()
    from traittutor.services.parsing.engines._install import get_background_job_manager
...
async def start_mineru_models_download(payload: MinerUModelDownloadPayload):
    ...
    """
    _require_settings_admin()
    from traittutor.services.parsing.engines.mineru.models import (
...
async def mineru_models_download_status(cursor: int = 0):
    _require_settings_admin()
    from traittutor.services.parsing.engines.mineru.models import get_model_download_manager
...
async def test_mineru_connection(payload: MinerUSettingsUpdate):
    ...
    _require_settings_admin()
    from traittutor.services.parsing.engines.mineru.cloud import verify_credentials
    from traittutor.services.parsing.engines.mineru.config import MinerUConfig, MinerUError

    if payload.mode == "local":
        from traittutor.services.parsing.engines.mineru.backend import (
            local_cli_probe,
            local_cli_version,
        )
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    Thin HTTP surface over provider model discovery so the settings UI can
    populate a model picker from ``base_url`` + ``api_key`` instead of making
    the user type model IDs by hand.
    """
    _require_settings_admin()
    from traittutor.services.llm.model_discovery import fetch_models as fetch_llm_models
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    _tour_store().save("state", cache)

    return {
        "status": "completed",
        "message": "Configuration saved. TraitTutor will restart shortly.",
        "launch_at": launch_at,
