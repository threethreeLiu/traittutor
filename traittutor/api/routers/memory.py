...
async def reset_doc(layer: str, key: str):
    ...
    from traittutor.services.memory.consolidator.runs import get_run_manager
    ...
    from traittutor.services.memory.consolidator import meta as meta_mod
...
def _runner_for(req: RunStartRequest):
    ...
    from traittutor.services.memory.consolidator import (
        run_audit,
        run_dedup,
        run_merge,
        run_update,
...
async def start_run(req: RunStartRequest):
    ...
    from traittutor.services.memory.consolidator.runs import (
        RunBusyError,
        get_run_manager,
    )
...
async def get_doc_lines(layer: str, key: str):
    ...
    lyr = _validate_layer(layer)
    _validate_doc_key(lyr, key)
    from traittutor.services.memory import paths
    from traittutor.services.memory.consolidator.line_doc import render_view
    from traittutor.services.memory.document import Document, parse
...
async def get_memory_settings_endpoint():
    ...
    from traittutor.services.memory.settings import memory_settings_dict
...
async def get_trace(surface: str, limit: int = 200, offset: int = 0):
    ...
    from traittutor.services.memory.trace import iter_since
...
async def get_snapshot(surface: str):
    ...
    """
    surf = _validate_surface(surface)
    from traittutor.services.memory import snapshot as snap
