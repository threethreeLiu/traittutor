...
from traittutor.services.config import resolve_search_runtime_config
...
async def get_runtime_topology():
    ...
    This makes the canonical runtime explicit for operators and frontend code.
    """
    return {
        "primary_runtime": {
            "transport": "/api/v1/ws",
        ...
        "capability_routes": ["chat", "solve", "question", "research"],
        "isolated_subsystems": [
        ...
        response = (
            await get_gateway().complete(
                GatewayRequest(
                    ...
                    purpose="system:llm-connection-test",
                    messages=(
                        GatewayMessage(role="system", content=system_prompt),
                        GatewayMessage(role="user", content=test_prompt),
                    ),
                    temperature=0.1,
                    max_tokens=200,
...
async def test_search_connection():
    ...
    try:
        ...
        result = web_search("TraitTutor health check", provider=search_config.provider)
        response_time = (time.time() - start_time) * 1000
