...
from traittutor.agents.notebook import NotebookSummarizeAgent
from traittutor.agents.notebook.summarize_agent import NotebookSummaryCancelled
from traittutor.services.llm import clean_thinking_tags
from traittutor.services.notebook import notebook_manager
...
async def _stream_add_record_with_summary(
    ...
    except NotebookSummaryCancelled:
        # A Gateway cancellation is not a successful partial summary.  The
        # only write lives after the stream loop, so ending here preserves the
        # existing no-partial-record disconnect semantics.
        return
