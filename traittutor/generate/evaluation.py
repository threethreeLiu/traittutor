...
# Courseware components that are teaching-support scaffolding rather than
# material assertions: a goal map projects milestones from the arrangement,
# reflection/progress/calibration prompts steer strategy, and a review queue
# schedules recall. None of them states a material fact, so the citation and
# grounding gates do not apply to a single-component generation of these types.
SUPPORT_COMPONENT_TYPES: frozenset[str] = frozenset(
    {
        "goal_map",
        "reflection_prompt",
        "progress_checkpoint",
...
def _generation_units(generation_type: str, output: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    if generation_type == "courseware":
        sections = _as_mappings(output.get("sections"))
        if sections:
            return sections
        # Single-component generation (e.g. goal_map) has no concept_explanation
        # sections; project the generated component itself as the unit.
        ...
        if isinstance(component, Mapping):
            ...
            body = _clean_text(
                props.get("body_markdown")
                or props.get("prompt")
                or props.get("front")
                # Media components (visual/video/audio) carry no prose; their
                # text alternative is the learner-visible content.
                or props.get("a11y_label")
                or "\n".join(
                    str(part).strip()
                    for part in (props.get("milestones") or [])
                    if str(part).strip()
                )
            )
            if title or body:
                # Carry the component's chunk-id references into the projected
                # unit: single-component runs (worked_example, audio, visual,
                # video, ...) have no concept_explanation section with
                # ``references``, so without this the grounding gate reports
                # missing_citations and fails every such generation.
                references = [
                    str(reference)
                    for reference in (props.get("concept_refs") or [])
                    if isinstance(reference, str) and reference.strip()
                ]
                unit: dict[str, Any] = {"title": title, "core_content": body}
                if references:
...
def _evaluate_structure(
    ...
    if generation_type == "courseware":
        ...
        support_type = _support_component_type(output)
        if not support_type:
            # Full lessons need several sections and renderable markdown; a
            # single support component (goal map etc.) intentionally has
            # neither and must not be penalized for it.
            if len(units) < 2:
                score -= 20
                _add_issue(
                    issues,
...
def _material_chunk_ids(
    material: Mapping[str, Any] | Sequence[Mapping[str, Any]] | None,
) -> set[str]:
    """Collect the chunk ids the model is told to cite.

    Lessons cite their sources with supplied chunk ids (``references``), not
    quoted text. The grounding gate must therefore accept an exact chunk-id
    match — a token-overlap test against the chunk text would always fail the
    citation and send every real generation to manual review.
    """
    if material is None:
        return set()
    candidates: list[Mapping[str, Any]] = []
    if isinstance(material, Mapping):
        ...
        candidates = _as_mappings(material)
    return {
        _normalize_for_match(str(candidate.get("chunk_id") or ""))
        for candidate in candidates
...
def _reference_matches_material(
    reference: Any,
    ...
    if len(snippet) < 4:
        return False
    if chunk_ids and snippet in chunk_ids:
        return True
...
def _support_component_type(output: Mapping[str, Any]) -> str:
    """Return the generated support-component type, or '' for full artifacts."""
    component = output.get("component")
    if not isinstance(component, Mapping):
        return ""
    component_type = _clean_text(component.get("component_type"))
    return component_type if component_type in SUPPORT_COMPONENT_TYPES else ""
...
def _evaluate_grounding(
    ...
    support_type = _support_component_type(output)
    if support_type:
        # Teaching-support components state no material facts (invariant #8);
        # their milestones/prompts come from the arrangement, not the material,
        # so a citation gate would only false-fail them.
        return EvaluationScore(100, (f"{support_type} 是教学支持组件，无需材料引用核验。",))
    units = _generation_units(generation_type, output)
    ...
    if not units:
        return EvaluationScore(0)
    chunk_ids = _material_chunk_ids(material)
    ...
    for unit in units:
        ...
        if any(
            _reference_matches_material(reference, material_texts, chunk_ids)
