...
def _stable_material_id(
    source_type: str, source_id: str, title: str, chunks: list[dict[str, Any]]
) -> str:
    seed = "|".join(
        [
            source_type,
...
def _metadata_summary(
    metadata: Mapping[str, Any] | None, chunks: list[dict[str, Any]]
) -> dict[str, Any]:
...
def build_material_abstraction(
    *,
    ...
    chunks = [
        chunk.to_dict() if hasattr(chunk, "to_dict") else dict(chunk) for chunk in resolved.chunks
    ]
    source_refs = []
    for chunk in chunks:
        citation_value = chunk.get("citation")
    ...
    return MaterialAbstraction(
        material_id=_stable_material_id(
            str(resolved.source_type), str(resolved.source_id), str(resolved.title), chunks
        ),
...
def build_learning_targets(
    *,
    ...
    signals = [
        item for item in context.get("relevant_concept_signals") or [] if isinstance(item, Mapping)
    ]
    targets = []
