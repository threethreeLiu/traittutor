...
async def generate_learning_visual(
    ...
    *,
    ...
    Single attempt by contract: retry/quota rotation/cancellation belong to
    the provider layer (Gateway / imagegen adapter), not to this caller —
    a caller-side retry loop here would re-issue provider spend after the
    bounded policy already gave up. The trace keeps the ``attempts`` list
    shape (one entry) so downstream consumers keep parsing.
    """
    ...
    trace: dict[str, Any] = {
        ...
        "prompt": prompt,
        "placement": "section"
        if prompt_source.get("kind") == "courseware"
        else prompt_source.get("kind"),
    ...
    try:
        ...
        asset_key = re.sub(
            r"[^a-zA-Z0-9_-]+",
            "-",
            _clean(prompt_source.get("asset_key"), limit=80),
        ).strip("-")
        filename = (
            f"learning-visual-{asset_key}.{suffix}" if asset_key else f"learning-visual.{suffix}"
        )
        ...
        trace["attempts"].append({"attempt": 1, "status": "completed"})
        ...
        trace["attempts"].append({"attempt": 1, "status": "failed", "message": str(exc)})
        trace.update({"status": "failed", "message": str(exc)})
    return trace


async def attach_hard_question_visuals(
    result: dict[str, Any],
    *,
    generation_id: str,
) -> dict[str, Any]:
    """Generate and bind one source-bounded image for every hard quiz item.

    The validated text question remains canonical. Image generation is an
    optional function call after validation, so a provider failure never drops
    the question or changes its answer, grading, or source references.
    """
    items = result.get("items")
    if result.get("kind") != "quiz" or not isinstance(items, list):
        return {"status": "skipped", "reason": "not_quiz", "questions": []}

    hard_questions = [
        (index, item)
        for index, item in enumerate(items, start=1)
        if isinstance(item, dict) and str(item.get("difficulty") or "").lower() == "hard"
    ]
    if not hard_questions:
        return {"status": "skipped", "reason": "no_hard_questions", "questions": []}

    semaphore = asyncio.Semaphore(_QUESTION_IMAGE_CONCURRENCY)

    async def generate_one(index: int, item: dict[str, Any]) -> dict[str, Any]:
        question_id = str(item.get("question_id") or index)
        node_name = _clean(item.get("node_name"), limit=120)
        question = _clean(item.get("question"), limit=500)
        async with semaphore:
            trace = await generate_learning_visual(
                {
                    "kind": "quiz",
                    "title": node_name or str(result.get("title") or "Hard question"),
                    "items": [{"question": question}],
                    "visual_targets": [
                        {
                            "concept_id": str(item.get("node_id") or question_id),
                            "label": node_name or question,
                        }
                    ],
                    "slr_visual_reason": "hard question visual support",
                    "asset_key": f"question-{question_id}",
                },
                generation_id=generation_id,
            )
        asset = trace.get("asset")
        if isinstance(asset, dict):
            item.setdefault("images", []).append(asset)
        return {
            "question_id": question_id,
            "difficulty": "hard",
            "status": trace.get("status", "failed"),
            **({"message": trace["message"]} if trace.get("message") else {}),
        }

    traces = await asyncio.gather(*(generate_one(index, item) for index, item in hard_questions))
    completed = sum(trace["status"] == "completed" for trace in traces)
    return {
        "status": "completed" if completed == len(traces) else "degraded",
        "reason": "hard_question_difficulty",
        "requested": len(traces),
        "completed": completed,
        "questions": traces,
    }
