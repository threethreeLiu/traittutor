...
class MaterialResolver:
    ...
    def _resolve_knowledge(self, reference: MaterialReference) -> ResolvedMaterial:
        ...
        if not raw_dir.is_dir():
            raise MaterialResolutionError(f"Knowledge base '{kb_ref}' has no raw material directory")
...
def _resolve_raw_path(raw_root: Path, relative_path: str) -> Path:
    ...
    try:
        ...
        raise MaterialResolutionError("Knowledge file path must stay inside the selected knowledge base") from exc
...
def _chunk_id(
    ...
    anchor = "\x1f".join(
        [
            source_type,
            source_id,
            str(locator.get("path") or locator.get("record_id") or locator.get("attachment_id") or ""),
