...
DEFAULT_FLASHCARDS_PER_BATCH = 5
...
class Flashcard(BaseModel):
    ...
    def _require_atomic_recall(self) -> Flashcard:
        if is_source_metadata_assessment(self.front):
            raise ValueError("flashcard must test subject knowledge, not source metadata")
...
class FlashcardBatch(BaseModel):
    ...
    items: list[Flashcard] = Field(min_length=1)
...
def plan_flashcard_batches(
    ...
    *,
    ...
    cards_per_batch: int = DEFAULT_FLASHCARDS_PER_BATCH,
    ...
    if cards_per_batch < 1:
        raise ValueError("cards_per_batch must be at least 1")
...
def validate_flashcard_payload(
    ...
    Reference repair runs first (mirroring the quiz path): models frequently
    paraphrase the quoted evidence or drift on source/chunk ids, and repairing
    those unambiguously keeps one minor formatting slip from failing the whole
    batch. Unknown or ambiguous references are left untouched and still fail.
    """

    try:
        normalized_chunks = coerce_grounding_chunks(chunks)
        batch = FlashcardBatch.model_validate(repair_payload_references(payload, normalized_chunks))
