"""Local Web launcher for the installed TraitTutor app."""
...
from traittutor.runtime.home import TRAITTUTOR_HOME_ENV, PACKAGE_ROOT, get_runtime_home
...
def _reset_runtime_singletons() -> None:
    """Make a just-selected TRAITTUTOR_HOME visible to path/config singletons."""
    try:
...
def start(home: str | Path | None = None) -> None:
    ...
    common_env[TRAITTUTOR_HOME_ENV] = str(runtime_home)
    ...
    common_env["TRAITTUTOR_API_BASE_URL"] = backend_url
    common_env["TRAITTUTOR_AUTH_ENABLED"] = "true" if auth_enabled else "false"
    ...
    backend_cmd = [
        ...
        # middleware (traittutor/api/main.py) surfaces only non-200s, so routine
