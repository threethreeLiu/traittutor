...
from traittutor.core.tool_protocol import BaseTool, ToolDefinition, ToolPromptHints
from traittutor.tools.builtin import BUILTIN_TOOL_TYPES, TOOL_ALIASES
from traittutor.tools.prompting import ToolPromptComposer
    ...
    def load_builtins(self) -> None:
        ...
        for tool_type in BUILTIN_TOOL_TYPES:
            ...
            if tool.name in self._tools:
                continue
            self.register(tool)
        # Mastery tools pull in the learning policy and its state stores.
        # Importing them from ``tools.builtin`` would close a bootstrap cycle
        # through PathService -> runtime registry -> builtin tools.  Loading
        # them only once the registry is instantiated preserves the same
        # global registration contract without making generic imports depend
        # on mastery storage.
        from traittutor.capabilities.mastery.tools import MASTERY_TOOL_TYPES

        for tool_type in MASTERY_TOOL_TYPES:
            try:
                tool = tool_type()
            except Exception:
                logger.warning("Failed to instantiate mastery tool %s", tool_type, exc_info=True)
                continue
