...
mastered when the evidence clears its threshold. :func:`next_objective`
normally keeps returning the same objective, but temporarily defers it after
two trusted failures so recovery cannot become an unbounded drill loop.
Advancement is still *computed from what is mastered*, never tracked by a stage counter. Objective ordering
...
from traittutor.learning_model.mastery_read_view import (
...
# canonical BKT estimate before the objective unlocks. ~0.9 mirrors Alpha
# School's "90% before you advance"; uncalibrated state cannot clear the gate.
...
def is_mastered(
    ...
    kp: KnowledgePoint,
    *,
    mastery_read_view: MasteryReadView | None = None,
) -> bool:
    ...
    * CONCEPT / DESIGN: a recorded qualitative pass (``mastery_assess``).
    ...
    canonical = _canonical_decision(kp, mastery_read_view)
    probability = canonical.mastery_probability if canonical is not None else None
    return probability is not None and probability >= gate_threshold(kp.type)


def _canonical_display(
    kp: KnowledgePoint,
    mastery_read_view: MasteryReadView | None,
) -> MasteryReadResult | None:
    """Resolve a quantitative public read inside an explicit identity partition."""
    if mastery_read_view is None or kp.type in QUALITATIVE_TYPES:
        return None
    return mastery_read_view.read(kp.id)


def _canonical_decision(
    kp: KnowledgePoint,
    mastery_read_view: MasteryReadView | None,
) -> MasteryDecisionState | None:
    if mastery_read_view is None or kp.type in QUALITATIVE_TYPES:
        return None
    return mastery_read_view.read_internal(kp.id)
...
def display_mastery(
    ...
    *,
    ...
    """Return the private calibrated estimate used by deterministic gates.

    Quantitative points require an identity-bound BKT view. Missing or
    uncalibrated evidence remains unknown; qualitative points use only the
    explicit Feynman-style gate and are never folded into BKT.
    """
    if kp.type in QUALITATIVE_TYPES:
        return _QUALITATIVE_PASS_DISPLAY if progress.qualitative_mastery.get(kp.id, False) else 0.0
    canonical = _canonical_decision(kp, mastery_read_view)
    return canonical.mastery_probability if canonical is not None else None
...
def objective_status(
    ...
    *,
    ...
    if is_mastered(progress, kp, mastery_read_view=mastery_read_view):
        return "mastered"
    canonical = _canonical_display(kp, mastery_read_view)
    seen = (
        any(a.knowledge_point_id == kp.id for a in progress.quiz_attempts)
        or kp.id in progress.qualitative_mastery
        or bool(canonical and canonical.verified_observation_count)
    )
...
def due_reviews(progress: LearningProgress, *, now: float | None = None) -> list[ReviewTask]:
    ...
    due = [
        ...
        and task.knowledge_point_id not in progress.deferred_knowledge_points
    ]
...
class NextStep:
    ...
    * ``recovery_pause`` — every remaining objective is temporarily deferred;
      wait for an explicit learner request before resuming one.
    ...
    *,
    ...
    if pending is not None:
        ...
        return NextStep(
            ...
            status=(
                objective_status(progress, kp, mastery_read_view=mastery_read_view)
                if kp
                else "learning"
            ),
            gate=_gate_kind(kp) if kp else "",
            mastery=(
                display_mastery(progress, kp, mastery_read_view=mastery_read_view) if kp else 0.0
            ),
    ...
    for module in sorted(progress.modules, key=lambda m: m.order):
        for kp in module.knowledge_points:
            if is_mastered(progress, kp, mastery_read_view=mastery_read_view):
                continue
            if kp.id in progress.deferred_knowledge_points:
                continue
            status = objective_status(
                progress,
                kp,
    ...
    for module in sorted(progress.modules, key=lambda m: m.order):
        for kp in module.knowledge_points:
            if kp.id not in progress.deferred_knowledge_points:
                continue
            if is_mastered(progress, kp, mastery_read_view=mastery_read_view):
                continue
            return NextStep(
                action="recovery_pause",
                module_id=module.id,
                module_name=module.name,
                knowledge_point_id=kp.id,
                knowledge_point_name=kp.name,
                ...
                threshold=gate_threshold(kp.type),
                reason=(
                    "This objective is temporarily paused after two consecutive "
                    "trusted failures. Resume only after the learner explicitly "
                    "asks to continue trying it."
                ),
                deferred=True,
            )
    ...
    for module in sorted(progress.modules, key=lambda m: m.order):
        ...
        for kp in module.knowledge_points:
            ...
            item: dict[str, object] = {
                ...
                "type": kp.type.value,
                "status": status,
                "deferred": kp.id in progress.deferred_knowledge_points,
            }
            if canonical is not None:
                item.update(
                    {
                        "evidence_state": canonical.evidence_state,
                        "change_signal": canonical.change_signal,
                        "verified_observation_count": canonical.verified_observation_count,
                        "model_version": canonical.model_version,
                        "stage_policy_version": canonical.stage_policy_version,
                        "mastery_updated_at": canonical.updated_at,
                    }
                )
            kps_out.append(item)
        modules_out.append(
