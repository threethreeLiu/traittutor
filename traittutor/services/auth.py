...
from traittutor.services.config import load_auth_settings, load_integrations_settings
...
def create_initial_admin(username: str, plain_password: str) -> dict | None:
    ...
    record = save_initial_admin(username, hash_password(plain_password), AUTH_USERNAME, AUTH_PASSWORD_HASH)
...
def decode_token(token: str) -> TokenPayload | None:
    ...
    if POCKETBASE_ENABLED:
        from traittutor.services.pocketbase_client import validate_pb_token
    ...
    try:
        ...
        return TokenPayload(username=str(username), role=str(record.get("role") or "user"), user_id=user_id)
    except JWTError:
        return None
...
def authenticate_pb(username: str, password: str) -> tuple[TokenPayload, str] | None:
    ...
    <username>@traittutor.local to match the email used at registration.
    """
    try:
