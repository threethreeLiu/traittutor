...
class BaseEmbeddingAdapter(ABC):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.api_key: str | None = str(api_key) if api_key is not None else None
        self.base_url: str = str(config.get("base_url") or "")
        self.api_version: str | None = str(api_version) if api_version is not None else None
        ...
        self.request_timeout: float = float(config.get("request_timeout", 60))
        self.extra_headers: dict[str, str] = {
            str(key): str(value) for key, value in (config.get("extra_headers") or {}).items()
        }

    def _should_send_openai_dimensions(self, model_name: str | None) -> bool:
        """Apply the shared OpenAI-style ``dimensions`` request policy."""
        if self.send_dimensions is not None:
            return self.send_dimensions
        if not model_name:
            return False
        normalized = model_name.lower()
        return normalized.startswith("text-embedding-3") or any(
            family in normalized for family in ("qwen3-embedding", "qwen3-vl-embedding")
        )
