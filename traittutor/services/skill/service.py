...
* builtin — shipped with the product under ``traittutor/skills/builtin/``,
...
The unified SQLite store holds the canonical user-managed tag vocabulary and
hub provenance independently of the skill document files.

Behaviour/voice settings are not skills. TraitTutor stores them as a typed,
versioned :mod:`traittutor.tutor_persona` profile; legacy free-text persona
prompts are archived and never injected into production prompts.
"""
...
# Builtin skills shipped inside the package and mounted by the product chat loop.
...
class SkillService:
    ...
    def __init__(
        ...
        self._path_service = get_path_service() if root is None else None

    def _state_store(self) -> SectionedRecordStore:
        return SectionedRecordStore(
            "skill_state",
            get_current_user().id,
            schema_version=1,
            path_service=self._path_service,
            db_path=None if self._path_service is not None else self._root / "traittutor.sqlite3",
        )
    ...
    def _read_tag_vocab(self) -> list[str]:
        record = next(iter(self._state_store().snapshot()["settings"]), None)
    ...
    def _write_tag_vocab(self, tags: list[str]) -> None:
        ...
        adapter = self._state_store()
        with adapter.locked() as payload:
            payload["settings"] = [
                {
                    "settings_id": "skill-tags",
                    "owner_id": owner,
    ...
    def _ensure_initialized_vocab(self) -> list[str]:
        ...
        existed = bool(self._state_store().snapshot()["settings"])
        if not existed:
    ...
    def install_tree(
        ...
        * ``origin`` (hub provenance) is recorded in ``.hub-lock.json``.
        ...
        tags = self._validate_tag_list(
            (raw_tags if isinstance(raw_tags, list) else []) + list(extra_tags or [])
        )
    ...
    def _read_hub_lock(self) -> dict[str, dict[str, Any]]:
        return {
            str(item["name"]): dict(item.get("origin") or {})
            for item in self._state_store().snapshot()["origins"]
            if item.get("name") and isinstance(item.get("origin"), dict)
        }
    ...
    def _write_hub_lock(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        with adapter.locked() as payload:
            payload["origins"] = [
                {"name": name, "owner_id": owner, "origin": origin} for name, origin in data.items()
            ]
            adapter.replace_all(payload)
