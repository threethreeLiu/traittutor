...
A hub skill is the same artefact TraitTutor already speaks natively — a
...
# TraitTutor ships pointing at EduHub, its native open skill registry: a bare
# ``install <slug>`` with no ``<hub>:`` prefix resolves here.
DEFAULT_HUB = "eduhub"
_CLAWHUB_BASE_URL = "https://clawhub.ai/api/v1"
_EDUHUB_BASE_URL = "https://eduhub.traittutor.info/api/v1"
...
class ClawHubProvider:
    ...
    def fetch(self, slug: str, *, version: str | None = None) -> FetchedSkill:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="traittutor-skill-"))
        try:
    ...
    def _listing_from_row(row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        owner = _json_object(row.get("owner"))
        return {
            "slug": slug,
    ...
    def catalog(
        ...
        TraitTutor-native browser renders without a second round-trip.
        """
        if query.strip():
    ...
    def detail(self, slug: str) -> dict[str, Any]:
        ...
        skill = _json_object(payload.get("skill")) or payload
        ...
        dist = _json_object(payload.get("distTags"))
        ...
        if not isinstance(keywords, list):
            keywords = _json_array(skill.get("tags"))
...
def _load_hub_config() -> dict[str, Any]:
    """Read the whole skill-hub settings document from canonical SQLite."""
    data = SQLiteDocumentStore(
        LOCAL_ADMIN_ID,
        namespace="skill-hubs",
