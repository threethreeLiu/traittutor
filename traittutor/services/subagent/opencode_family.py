...
HTTP server (managed by :mod:`traittutor.services.subagent.opencode_server`),
...
from traittutor.services.subagent.opencode_server import acquire_server
from traittutor.services.subagent.process import probe_version
...
class OpencodeFamilyBackend(SubagentBackend):
    ...
    async def _create_session(self, client: httpx.AsyncClient) -> str:
        response = await client.post("/session", json={"title": "TraitTutor consult"})
        response.raise_for_status()
        data = response.json()
