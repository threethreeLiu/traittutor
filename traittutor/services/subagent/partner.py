...
*partner session key*. The first consult of a TraitTutor chat session has none,
so we mint a fresh ``dt-…`` key and return it; the cross-turn registry
(:mod:`traittutor.services.subagent.sessions`) remembers it against
(chat session, connection), so every later consult in the same TraitTutor chat —
within one turn or across turns — resumes the SAME partner session. The partner
page then sees one complete history session per TraitTutor chat, titled from the
...
from traittutor.services.subagent.base import OnEvent, SubagentBackend
from traittutor.services.subagent.config import BackendConfig
from traittutor.services.subagent.types import (
    EVENT_ERROR,
    EVENT_REASONING,
    EVENT_TEXT,
    EVENT_TOOL,
...
if TYPE_CHECKING:  # avoid importing the core/partner packages at module load
    from traittutor.core.stream import StreamEvent
...
class PartnerBackend(SubagentBackend):
    ...
    async def consult(
        ...
        *,
        ...
        # a TraitTutor chat → mint a stable, colon-free key; the registry threads
