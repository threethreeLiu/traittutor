...
Pydantic models + SQLite persistence for the deployment's MCP server registry.

The config is deployment-global in the admin workspace database: every user
talks to the same set of connected servers.
...
def _mcp_store() -> SQLiteDocumentStore:
...
def save_mcp_config(config: MCPConfig) -> None:
    _mcp_store().save("servers", config.model_dump(mode="json"))
