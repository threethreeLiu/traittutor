...
from traittutor.runtime.home import get_runtime_data_root
...
class RAGService:
    ...
    TraitTutor's authoritative KB config, with metadata as a legacy fallback.
    """
    ...
    def _resolve_provider(self, kb_name: Optional[str]) -> str:
        """Pick the provider for ``kb_name`` from TraitTutor's binding."""
    ...
    async def search(
        ...
        **kwargs,
        ...
        with self._capture_raw_logs(event_sink):
            ...
            # Trail is append-only evidence. It never updates Reflection,
            # Compass, canonical memory, or BKT from a retrieval action.
            try:
    ...
    def _capture_raw_logs(self, event_sink):
        ...
        from traittutor.logging import ProcessLogEvent, capture_process_logs
        from traittutor.logging.formatters import ContextFilter
        ...
        def should_skip_noisy_retrieve_log(event: ProcessLogEvent) -> bool:
            ...
            if logger_name == "nano-vectordb" and (
                ...
                return True
            if (
                logger_name.startswith("traittutor.services.embedding.")
                and (
            ...
            def emit(self, record: logging.LogRecord) -> None:
                try:
                    ...
                    try:
                        ...
                        asyncio.run_coroutine_threadsafe(_await_result(), target_loop)
                        return
