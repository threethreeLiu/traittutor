...
document-parse bridge (``traittutor/services/parsing``), so the engine the user
...
from traittutor.services.llm.client import get_llm_client
...
class LlamaIndexDocumentLoader:
    ...
    def _parse_document(self, file_path: Path) -> tuple[str, list[_ImageSource]]:
        ...
        """
        from traittutor.services.parsing import ParserError, get_parse_service
    ...
    async def _load_image_nodes(self, sources: list[_ImageSource]) -> list[ImageNode]:
        ...
        # Images and their descriptions/embeddings are produced by the same
        # pipeline pass, so lengths match; strict=False preserves that contract.
        for source, description, embedding in zip(embedded, descriptions, embeddings, strict=False):
