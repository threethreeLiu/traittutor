...
This module is the single seam between TraitTutor's LlamaIndex pipeline and the
...
def load_index(storage_dir: Path) -> Any:
    ...
    if detect_backend(storage_dir) == BACKEND_FAISS:
        ...
        # ``cosine_cls`` is a dynamically built FaissVectorStore subclass;
        # its classmethod is not visible to the type checker.
        vector_store = cast(Any, cosine_cls).from_persist_dir(str(storage_dir))
