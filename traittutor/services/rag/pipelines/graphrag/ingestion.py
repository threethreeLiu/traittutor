...
(its optional ``markitdown`` reader is just a converter). So TraitTutor owns the
"document → text" step and hands GraphRAG ready ``.txt`` files. We deliberately
reuse TraitTutor's existing extraction primitives here so multimodal/parsing
stays a TraitTutor-side concern (matching how the LlamaIndex pipeline already
