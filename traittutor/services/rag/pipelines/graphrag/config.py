"""Bridge TraitTutor's runtime config into a GraphRAG ``settings.yaml``.
...
TraitTutor's already-resolved LLM + embedding runtime config and let
...
* GraphRAG talks to its models via LiteLLM with ``model_provider: openai`` + a
  custom ``api_base`` — which is exactly how TraitTutor reaches any
...
class GraphRagNotConfiguredError(RuntimeError):
    """Raised when TraitTutor's LLM / embedding config can't back GraphRAG."""
...
def is_graphrag_available() -> bool:
    ...
    extra: ``pip install 'traittutor[graphrag]'``. Until it is installed the
...
def query_config_from_settings() -> GraphRagQueryConfig:
    ...
    try:
        from traittutor.services.config import load_graphrag_settings
...
def build_settings(*, llm_cfg: Any = None, embedding_cfg: Any = None) -> dict[str, Any]:
    """Assemble the GraphRAG ``settings.yaml`` payload from TraitTutor config.

    ``llm_cfg`` / ``embedding_cfg`` are injectable for tests; in production they
    are resolved from TraitTutor's catalog. Raises
    :class:`GraphRagNotConfiguredError` if either side has no usable model.
    """
    if llm_cfg is None:
        from traittutor.services.config import resolve_llm_runtime_config
    ...
    return {
        ...
        # Plain-text input: TraitTutor's ingestion writes parsed ``.txt`` files
        ...
        # GraphRAG/LanceDB defaults to 3072 dimensions; TraitTutor must stamp the
