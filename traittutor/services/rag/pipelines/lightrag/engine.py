...
fix. All imports are lazy so TraitTutor runs fine without the optional dependency
installed.

A RAG-Anything instance is built from TraitTutor's LLM/vision/embedding adapters
...
def build_rag(working_dir: Path) -> Any:
    ...
    # TraitTutor always feeds RAG-Anything a pre-parsed ``content_list`` (the
    # parse layer runs upstream via TraitTutor's own ParseService), so
