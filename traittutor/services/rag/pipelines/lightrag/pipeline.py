...
Documents are turned into a MinerU-style ``content_list`` by TraitTutor's shared
parse layer (``traittutor/services/parsing``) — the same bridge the question
...
from traittutor.services.rag.kb_paths import resolve_kb_dir
    ...
    def _ensure_available(self) -> None:
        if not lr_config.is_lightrag_available():
            raise lr_config.LightRagNotAvailableError(
                ...
                "`pip install 'traittutor[rag-lightrag]'` to use LightRAG knowledge bases."
            )
