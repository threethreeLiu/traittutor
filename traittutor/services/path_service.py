...
from traittutor.runtime.home import PACKAGE_ROOT, get_runtime_data_root
...
class PathService:
    ...
    def get_parse_cache_root(self) -> Path:
        ...
        ``(source_hash, parser_signature)`` — see ``traittutor/services/parsing``.
        """
    ...
    def get_traittutor_database_path(self) -> Path:
        """Return this authenticated workspace's unified business database path.

        Callers obtain the owning :class:`PathService` from the server-side user
        scope.  Keeping this resolution here prevents request payloads from
        selecting an arbitrary database file.
        """
        return self.get_workspace_dir() / "traittutor" / "traittutor.sqlite3"
    ...
    def _resolve_feature_root(self, feature: str) -> Path:
        ...
        if feature in {"memory", "notebook", "book"}:
    ...
    def ensure_all_directories(self) -> None:
        """Create only the roots required before the first domain write.

        Feature stores create their own directories on demand. Pre-creating the
        full historical tree left every fresh workspace populated with empty
        notebook, book, memory, and agent directories that carried no state.
        """
...
def get_path_service() -> PathService:
    try:
        from traittutor.multi_user.paths import get_current_path_service
