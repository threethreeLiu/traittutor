...
class GitHubCopilotProvider(OpenAICompatProvider):
    ...
    async def _load_stored_github_token(self) -> str | None:
        ...
        storage = FileTokenStorage(  # nosec B106 - token_filename is a file name, not a password.
            ...
            # app_name is oauth_cli_kit's *local* token-store namespace, NOT an
            # external identity: it must match the name under which the token was
            # written, so renaming it would hide an already-stored token. The
            # Copilot API itself is called as User-Agent "TraitTutor/1".
            app_name="nanobot",
