...
class AnthropicProvider(LLMProvider):
    ...
    def __init__(
        ...
        from traittutor.services.llm.utils import sanitize_url
    ...
    def _build_kwargs(
        ...
        # "none"/"minimal"/"minimum" mean thinking off on every family, not
        # just the effort-based ones: treating the truthy string "none" as
        # enabled silently turned the off-request into a 4k thinking budget on
        # non-effort-based models (e.g. MiniMax-M3), making "none" runs slower
        # than "low" instead of faster.
        thinking_enabled = (
            bool(reasoning_effort) and (reasoning_effort or "").lower() not in _THINKING_OFF_EFFORTS
        )
        ...
        if reasoning_effort == "adaptive":
            ...
            # the param entirely.
            if (reasoning_effort or "").lower() not in _THINKING_OFF_EFFORTS:
                kwargs["thinking"] = {"type": "adaptive"}
