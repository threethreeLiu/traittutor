...
from traittutor.services.llm.capabilities import disable_response_format_at_runtime
from traittutor.services.llm.openai_http_client import openai_client_kwargs
from traittutor.services.llm.provider_core.base import LLMProvider, LLMResponse, ToolCallRequest
from traittutor.services.llm.provider_core.openai_responses import (
...
if TYPE_CHECKING:
    from traittutor.services.provider_registry import ProviderSpec
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _parse(self, response: Any) -> LLMResponse:
        ...
        images = self._extract_message_images(response)
    ...
    @staticmethod
    def _extract_message_images(response: Any) -> list[dict[str, Any]]:
        """Collect image parts from a chat-completions response.

        OpenRouter-style image models return ``choices[].message.images``
        (``{"image_url": {"url": ...}}``); some variants nest them in the
        content-parts array instead. Both shapes are normalized to
        ``{"url": ...}`` dicts on :class:`LLMResponse.images`.
        """
