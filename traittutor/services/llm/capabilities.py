...
Usage:
    from traittutor.services.llm.capabilities import get_capability, supports_response_format
...
def supports_tools(binding: str, model: str | None = None) -> bool:
    ...
    if model:
        model_lower = model.lower()
        for pattern, overrides in sorted(MODEL_OVERRIDES.items(), key=lambda item: -len(item[0])):
            if model_lower.startswith(pattern) and "supports_tools" in overrides:
                return bool(overrides["supports_tools"])

    provider_caps = PROVIDER_CAPABILITIES.get(binding_lower, {})
    if "supports_tools" in provider_caps:
        return bool(provider_caps["supports_tools"])
    ...
    if spec.backend in {"anthropic", "openai_codex"}:
        return True
    return spec.backend == "openai_compat"
