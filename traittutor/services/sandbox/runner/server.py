...
(:class:`traittutor.services.sandbox.backends.RunnerSidecarBackend`).
...
Design constraints:
  ...
  * Defence in depth: the container already drops privileges (non-root
    ...
    — see ``Dockerfile.runner`` / ``compose.yaml``). On top of that we
...
Wire contract (must match ``RunnerSidecarBackend``):
  ...
  ``POST /exec``   -> request/response JSON described by the dataclasses in
                      :mod:`traittutor.services.sandbox.spec`. Request::
...
Mounts note:
  ...
  (``TRAITTUTOR_RUNNER_ALLOWED_WORKDIRS``) as defence in depth.
"""
...
_ALLOWED_WORKDIR_ROOTS = [
    ...
    for root in os.environ.get(
        "TRAITTUTOR_RUNNER_ALLOWED_WORKDIRS",
