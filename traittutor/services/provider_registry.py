"""Provider registry for TraitTutor LLM routing.
...
# Canonical tuple of supported LLM provider names. ``NANOBOT_LLM_PROVIDERS``
# below is kept as a back-compat alias: TraitTutor is derived from
# HKUDS/nanobot (MIT; see THIRD_PARTY_NOTICES.md) and this name is part of the
# package's public API, so removing it outright would break existing importers.
TRAITTUTOR_LLM_PROVIDERS: tuple[str, ...] = tuple(spec.name for spec in PROVIDERS)
NANOBOT_LLM_PROVIDERS = (
    TRAITTUTOR_LLM_PROVIDERS  # deprecated alias; prefer TRAITTUTOR_LLM_PROVIDERS
)
