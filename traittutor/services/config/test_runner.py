...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from traittutor.services.llm import clear_llm_config_cache
        ...
        run.emit("info", f"Gateway max tokens: {max_tokens}")
    ...
    async def _test_embedding(
        ...
        from traittutor.services.embedding.client import EmbeddingClient
        from traittutor.services.embedding.config import EmbeddingConfig
        ...
        probe_texts = [
            "TraitTutor embedding smoke test",
            "TraitTutor retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        run.emit("info", "Running search query: TraitTutor configuration health check")
        result = web_search("TraitTutor configuration health check", provider=provider)
        run.emit(
            "response",
    ...
    async def _test_tts(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from traittutor.services.config.provider_runtime import resolve_tts_runtime_config
        from traittutor.services.voice import synthesize_speech
        ...
        sample = "TraitTutor voice check. 这是一段语音合成测试。"
    ...
    async def _test_stt(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from traittutor.services.config.provider_runtime import resolve_stt_runtime_config
        from traittutor.services.voice import transcribe_audio
    ...
    async def _test_imagegen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from traittutor.services.config.provider_runtime import resolve_imagegen_runtime_config
    ...
    async def _test_videogen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from traittutor.services.videogen import probe_video
        ...
        run.emit("info", f"Request target: {resolved.base_url}")
        run.emit("info", "Submitting a short provider probe without waiting for the render.")
        task_id = await probe_video("A short educational test clip.", catalog=catalog)
        run.emit("response", "Video task accepted — connection is valid.", task_id=task_id)
