...
class ModelCatalogService:
    ...
    def _overlay_local_llm(self, catalog: dict[str, Any]) -> None:
        """Replace services.llm with code-defined models from models.local.yaml.

        No-op when the file is absent/empty so the JSON catalog remains the
        source of truth for llm in that case. Lazy import avoids cycles.
        """
        try:
            from traittutor.services.models.local_catalog import load_local_llm

            overlay = load_local_llm()
        except Exception:
            overlay = None
        if overlay is not None:
            catalog.setdefault("services", {})["llm"] = overlay

    def _read_existing_catalog(self) -> dict[str, Any]:
        loaded = self._documents.load("catalog", {})
        return loaded if isinstance(loaded, dict) else {}

    def save(self, catalog: dict[str, Any]) -> dict[str, Any]:
        normalized = deepcopy(catalog)
        self._normalize(normalized)
        # LLM models are code-defined in config/models.local.yaml and overlaid
        # on load(); never persist their keys/profiles to the JSON catalog.
        normalized.setdefault("services", {})["llm"] = _service_shell()
        self._documents.save("catalog", normalized)
        return normalized

    def apply(self, catalog: dict[str, Any] | None = None) -> dict[str, Any]:
        current = self.save(catalog or self.load())
        return {
            "catalog_path": str(self._documents.db_path),
