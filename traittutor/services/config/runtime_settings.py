...
# Document parsing settings. The parse layer (traittutor/services/parsing)
...
#
# Persisted only as ``document_parsing.json`` using the v2 engine structure.
...
_DEFAULT_MINERU_ENGINE: dict[str, Any] = {
    ...
    # separate conda) so its heavy deps never conflict with TraitTutor's.
    "local_cli_path": "",
...
# TraitTutor's VLM to describe images.
...
# Built-in text-only engine slice. It deliberately has no knobs.
...
# LightRAG retrieval knobs (LightRAG via RAG-Anything). ``top_k`` is the
...
IGNORE_PROCESS_OVERRIDES_ENV = "TRAITTUTOR_IGNORE_PROCESS_ENV_OVERRIDES"
...
class RuntimeSettingsService:
    """SQLite-backed runtime settings scoped to one workspace database.
    ...
    def __init__(
        ...
        *,
        ...
        database_path = (
            settings_dir.parent / "workspace" / "traittutor" / "traittutor.sqlite3"
            if settings_dir.name == "settings"
            else settings_dir / "traittutor.sqlite3"
        )
        self._documents = SQLiteDocumentStore(
            LOCAL_ADMIN_ID,
    ...
    def save_system(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        self._documents.save("system", payload)
        return payload
    ...
    def save_auth(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        self._documents.save("auth", payload)
        return payload
    ...
    def save_document_parsing(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        self._documents.save(DOCUMENT_PARSING_SETTINGS_NAME, payload)
        return payload

    def load_mineru(self, *, include_process_overrides: bool = True) -> dict[str, Any]:
        """Return the MinerU slice from canonical document-parsing settings."""
        slice_ = dict(
    ...
    def save_pageindex(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        self._documents.save("pageindex", payload)
        return payload
    ...
    def save_llamaindex(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        self._documents.save("llamaindex", payload)
        return payload
    ...
    def save_graphrag(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        self._documents.save("graphrag", payload)
        return payload
    ...
    def render_environment(self) -> dict[str, str]:
        ...
        return {
            ...
            "TRAITTUTOR_SANDBOX_ALLOW_SUBPROCESS": _bool_env(system["sandbox_allow_subprocess"]),
    ...
    def _load_or_create(
        ...
        display_name = f"{name} settings"
        loaded = self._documents.load(name, {})
        if not isinstance(loaded, dict):
            raise ValueError(f"Runtime setting {display_name} must contain an object")
        if loaded:
            unknown = sorted(set(loaded) - set(defaults))
            if unknown:
                raise ValueError(f"Unsupported keys in {display_name}: {', '.join(unknown)}")
            expected_version = defaults.get("version")
            if expected_version is not None and loaded.get("version") != expected_version:
                raise ValueError(f"Unsupported {display_name} version: {loaded.get('version')!r}")
            normalized = normalizer({**defaults, **loaded})
            if normalized != loaded:
                self._documents.save(name, normalized)
            return normalized
    ...
    def _apply_system_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("TRAITTUTOR_SANDBOX_ALLOW_SUBPROCESS"):
    ...
    def _apply_llamaindex_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        # (TRAITTUTOR_RAG_RETRIEVAL_PROFILE / RAG_RETRIEVAL_PROFILE); preserve it.
        payload = dict(settings)
        if value := (
            self._process_env_value("TRAITTUTOR_RAG_RETRIEVAL_PROFILE")
    ...
    def _normalize_document_parsing(self, settings: dict[str, Any]) -> dict[str, Any]:
        """Normalize the canonical v2 document-parsing structure."""
        settings = dict(settings)
        unknown = sorted(set(settings) - {"version", "engine", "engines"})
        if unknown:
            raise ValueError(f"Unsupported document-parsing keys: {', '.join(unknown)}")
        if settings.get("version") != 2:
            raise ValueError("document_parsing.version must be 2")

        raw_engines = settings.get("engines")
        if not isinstance(raw_engines, dict):
            raise ValueError("document_parsing.engines must be an object")
        unknown_engines = sorted(set(raw_engines) - _DOCUMENT_PARSING_ENGINES)
        if unknown_engines:
            raise ValueError(f"Unsupported document-parsing engines: {', '.join(unknown_engines)}")
        engines_in = dict(raw_engines)
        ...
        if engine not in _DOCUMENT_PARSING_ENGINES:
            raise ValueError(f"Unsupported document-parsing engine: {engine!r}")
    ...
    def _normalize_auth(self, settings: dict[str, Any]) -> dict[str, Any]:
        return {
            "version": 2,
            "enabled": _coerce_bool(settings.get("enabled"), True),
            ...
            "allow_public_registration": _coerce_bool(
                settings.get("allow_public_registration"), False
            ),
        }
...
def ensure_runtime_settings_files() -> None:
    """Create missing canonical runtime-settings files from safe defaults.
...
def get_chat_attachment_limits() -> ChatAttachmentLimits:
    ...
    chat uploads and the composer via the settings API) so edits apply to the
