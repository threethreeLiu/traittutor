"""Nanobot-style normalized runtime configuration for TraitTutor."""
...
from traittutor.services.model_selection import LLMSelection, apply_llm_selection_to_catalog
from traittutor.services.provider_registry import (
    NANOBOT_LLM_PROVIDERS,
    PROVIDERS,
    TRAITTUTOR_LLM_PROVIDERS,
    ProviderSpec,
...
# Video generation is restored narrowly for the Agnes endpoint verified by the
# learning-component flow. Do not advertise retired provider adapters here.
VIDEOGEN_PROVIDERS: dict[str, GenerationProviderSpec] = {
    "agnes": GenerationProviderSpec(
        label="Agnes AI (Video 2.0)",
        default_api_base="https://apihub.agnes-ai.com/v1",
        adapter="agnes",
    ...
    *,
    ...
    if _is_local_base_url(api_base):
        if api_base and "11434" in api_base:
            local_spec = find_by_name("ollama") or find_by_name("vllm")
        else:
            local_spec = find_by_name("vllm") or find_by_name("ollama")
...
def resolve_llm_runtime_config(
    ...
    *,
    ...
    """Resolve active LLM config with catalog-backed provider matching."""
...
def resolve_videogen_runtime_config(
    ...
    *,
    ...
    api_base = _as_str((profile or {}).get("base_url")) or spec.default_api_base
    api_key = _as_str((profile or {}).get("api_key"))
    # Agnes exposes image and video models behind the same account/API key.
    # Reuse the active Agnes image credential when the dedicated video profile
    # intentionally leaves its key empty; this avoids duplicating a secret in
    # two persisted catalog branches while keeping other providers isolated.
    if not api_key and provider == "agnes":
        image_profile = catalog_service.get_active_profile(loaded, "imagegen") or {}
        image_provider = _canonical_generation_provider(
            _as_str(image_profile.get("binding")), IMAGEGEN_PROVIDERS
        )
        if image_provider == "agnes":
            api_key = _as_str(image_profile.get("api_key"))
    if not api_key and spec.is_local:
        api_key = "sk-no-key-required"
...
def resolve_search_runtime_config(
    ...
    *,
    ...
    """Resolve active web-search config with catalog fallback behavior."""
    catalog_service = service or get_model_catalog_service()
    loaded = _load_catalog(catalog)
