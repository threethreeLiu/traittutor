...
# ``traittutor/services/config/__init__.py``) which resolves the path lazily.
...
class KnowledgeBaseConfigService:
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        try:
            loaded = KnowledgeStateStore(self.config_path.parent).load_config()
    ...
    def _save(self) -> None:
        ...
        KnowledgeStateStore(self.config_path.parent).save_config(self._config)
    ...
    def sync_from_metadata(self, kb_name: str, kb_base_dir: Path) -> None:
        metadata = KnowledgeStateStore(kb_base_dir).load_metadata(kb_name)
        if not metadata:
            return
        config: dict[str, Any] = {}
