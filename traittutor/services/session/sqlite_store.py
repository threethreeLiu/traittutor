...
def _current_owner_id() -> str:
    """Return the authenticated owner for this storage operation.

    SQLite normally isolates users by database path.  Keeping an owner column
    as a second, query-level boundary means a shared/test database cannot turn
    a caller-provided session or turn id into cross-user access.  The value is
    derived from the request ContextVar, never from a session payload.
    """
...
class SQLiteSessionStore:
    ...
    def __init__(self, db_path: Path | None = None) -> None:
        ...
        self.db_path = db_path or path_service.get_traittutor_database_path()
    ...
    def _initialize(self) -> None:
        with self._connect() as conn:
            conn.executescript(
                ...
                -- Quiz answer keys are private grading material.  They are
                -- captured at the server's stream boundary and are never
                -- reconstructed from a browser submission.
                CREATE TABLE IF NOT EXISTS server_quiz_items (
                    session_id TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE,
                    turn_id TEXT NOT NULL REFERENCES turns(id) ON DELETE CASCADE,
                    ...
                    created_at REAL NOT NULL,
                    updated_at REAL NOT NULL,
                    PRIMARY KEY (session_id, turn_id, question_id)
                );

                CREATE INDEX IF NOT EXISTS idx_server_quiz_items_turn
                    ON server_quiz_items(turn_id, question_id);
            ...
            # A legacy SQLite database predates multi-user session rows and
            # lived only in the admin workspace.  It has no trustworthy
            # per-row identity to recover, so bind it to the historical local
            # admin rather than silently making it visible to every account.
            conn.execute("UPDATE sessions SET owner_id = 'local-admin' WHERE owner_id = ''")
    ...
    def _create_session_sync(
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, '', 0)
                """,
                (resolved_id, owner_id, resolved_title[:100], now, now),
            )
            conn.commit()
        return {
            "id": resolved_id,
    ...
    def _get_active_turn_sync(self, session_id: str) -> dict[str, Any] | None:
        ...
        with self._connect() as conn:
            row = conn.execute(
                ...
                FROM turns t
                INNER JOIN sessions s ON s.id = t.session_id
                WHERE t.session_id = ? AND t.status = 'running' AND s.owner_id = ?
                ORDER BY t.updated_at DESC
                LIMIT 1
                """,
                (session_id, owner_id),
    ...
    def _update_turn_status_sync(self, turn_id: str, status: str, error: str = "") -> bool:
        ...
        with self._connect() as conn:
            cur = conn.execute(
                ...
                """,
                (status, error or "", now, finished_at, turn_id, owner_id),
            )
            conn.commit()
    ...
    def _add_message_sync(
        ...
        with self._connect() as conn:
            ...
            resolved_parent_id = int(parent_message_id) if parent_message_id is not None else None
    ...
    def _import_session_sync(
        ...
        with self._connect() as conn:
            existing = conn.execute(
                "SELECT preferences_json FROM sessions WHERE id = ? AND owner_id = ?",
                (session_id, owner_id),
            ).fetchone()
            ...
            for msg in messages:
                ...
                if cur.lastrowid is None:
                    raise RuntimeError("SQLite did not return an imported message id")
    ...
    # ── Server-held quiz items ──────────────────────────────────────

    def _upsert_server_quiz_items_sync(
        self,
        session_id: str,
        turn_id: str,
        items: list[dict[str, Any]],
    ) -> int:
        """Persist private generated quiz definitions for later grading.

        This deliberately accepts items only from the server stream runtime.
        Public quiz endpoints receive identifiers and learner answers, then
        resolve the authoritative definition from this table.
        """
        if not session_id or not turn_id or not items:
            return 0
        now = time.time()
        with self._connect() as conn:
            turn = conn.execute(
            ...
            if turn is None:
                raise ValueError("Quiz turn does not belong to session")
            saved = 0
            for raw in items:
                ...
                expected = str(raw.get("correct_answer") or "").strip()
                if not question_id or not question or not expected:
                    # A malformed generation must not become client-graded.
                    continue
                item: dict[str, Any] = {
                    ...
                    "concentration": str(raw.get("concentration") or ""),
                    # Only server-emitted metadata may be used as a future
                    # attribution hint; no browser payload reaches here.
                    "subject_id": str(raw.get("subject_id") or ""),
                    "kc_id": str(raw.get("kc_id") or raw.get("node_id") or ""),
                    # This is intentionally private.  It is only meaningful
                    # when the server generated the question for an existing
                    # LearningProgress; sessions must never be guessed as
                    # paths during later browser submission.
                    "learning_path_id": str(raw.get("learning_path_id") or ""),
                }
                conn.execute(
                    """
                    INSERT INTO server_quiz_items (
                        session_id, turn_id, question_id, question_json, created_at, updated_at
                    ) VALUES (?, ?, ?, ?, ?, ?)
                    ON CONFLICT(session_id, turn_id, question_id) DO UPDATE SET
                        question_json = excluded.question_json,
                        updated_at = excluded.updated_at
                    """,
                    (session_id, turn_id, question_id, _json_dumps(item), now, now),
                )
                saved += 1
            conn.commit()
        return saved

    async def upsert_server_quiz_items(
        self, session_id: str, turn_id: str, items: list[dict[str, Any]]
    ) -> int:
        return await self._run(self._upsert_server_quiz_items_sync, session_id, turn_id, items)

    def _get_server_quiz_item_sync(
        self, session_id: str, turn_id: str, question_id: str
    ) -> dict[str, Any] | None:
        with self._connect() as conn:
            row = conn.execute(
                """
                SELECT question_json
                FROM server_quiz_items
        ...
        if row is None:
            return None
        item = _json_loads(row["question_json"], {})
        return item if isinstance(item, dict) else None

    async def get_server_quiz_item(
        self, session_id: str, turn_id: str, question_id: str
    ) -> dict[str, Any] | None:
        return await self._run(self._get_server_quiz_item_sync, session_id, turn_id, question_id)
    ...
    def _create_category_sync(self, name: str) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            ...
            conn.commit()
        if cur.lastrowid is None:
            raise RuntimeError("SQLite did not return a notebook category id")
...
def get_sqlite_session_store() -> SQLiteSessionStore:
    db_path = get_path_service().get_traittutor_database_path().resolve()
    key = str(db_path)
