...
The canonical SQLite session store satisfies this protocol.
"""
...
class SessionStoreProtocol(Protocol):
    """Owner-scoped durable session boundary.

    Implementations derive the owner from the authenticated request context.
    ``session_id`` and ``turn_id`` are opaque references, never authority:
    owner-mismatched ``get_session``/``get_turn`` calls return ``None`` and
    mutating turn operations return ``False`` (or reject) without side
    effects. This keeps WebSocket replay, cancellation, and user-reply paths
    consistently owner-bound.
    """
