...
class BaseSessionManager(ABC):
    ...
    def _ensure_file(self) -> None:
        """Compatibility no-op; tables are created lazily by the adapter."""
        return  # explicit statement keeps this concrete hook non-abstract (B027)
    ...
    def _load_data(self) -> dict[str, Any]:
        sessions = [
            {key: value for key, value in item.items() if key not in {"storage_id", "module_name"}}
            for item in self._adapter().snapshot()["sessions"]
            if item.get("module_name") == self.module_name
        ]
        return {"version": "1.0", "sessions": sessions}

    def _save_data(self, data: dict[str, Any]) -> None:
        adapter = self._adapter()
        with adapter.locked() as payload:
            other_modules = [
                item for item in payload["sessions"] if item.get("module_name") != self.module_name
            ]
            current = [
                {
                    **session,
                    "module_name": self.module_name,
                    "storage_id": f"{self.module_name}:{session['session_id']}",
                }
                for session in data.get("sessions", [])
            ]
            payload["sessions"] = [*other_modules, *current]
