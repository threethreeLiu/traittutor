        ...
        try:
            ...
            if product_mode in {"learn", "assist"}:
                ...
                route_event = StreamEvent(
                    ...
                    content="Routing your request to a TraitTutor specialist.",
                )
                payload_event = await self._publish_live_event(execution, route_event)
                ...
                policy_event = StreamEvent(
                    ...
                    source="traittutor_agent",
                    stage="policy",
                    content=f"Using {agent_result.agent.replace('_', ' ')}.",
                    metadata={"intent": agent_result.intent.value, "policy": [item.model_dump() for item in agent_result.policy]},
                )
                assistant_events.append(await self._publish_live_event(execution, policy_event))
                ...
                assistant_events.append(payload_event)
                content_segments.append((None, content_event.content))
            else:
