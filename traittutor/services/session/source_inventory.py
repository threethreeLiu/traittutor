...
async def _load_lineage(
    ...
    for listed_message in all_msgs:
        mid = listed_message.get("id")
        if mid is not None:
            by_id[int(mid)] = listed_message
...
def _serialize_learning_artifact(
    ...
    """Serialize only learner-visible artifact fields for model context.

    Learning artifacts are referenced by the browser, but their answer faces,
    grading explanations, rubrics, and inactive hints remain server-held.  This
    serializer feeds both the prompt manifest and ``read_source``; keeping the
    allowlist here prevents either path from bypassing the Learning Canvas
    excerpt filter.
    """

    material_value = pack.get("material")
    material: dict[str, Any] = material_value if isinstance(material_value, dict) else {}
    lines = [
    ...
    if artifact_type == "courseware":
        ...
        if isinstance(sections, list):
            ...
            for idx, section in enumerate(sections, start=1):
                if not isinstance(section, dict):
                    continue
                title = str(
                    section.get("title") or section.get("section_title") or f"Section {idx}"
                )
        ...
        if isinstance(items, list):
            ...
            for idx, item in enumerate(items, start=1):
                ...
                if question or option_text:
                    rendered.append(f"{idx}. Question: {question}\n{option_text}".strip())
            if rendered:
                lines.extend(["## Quiz", "\n\n".join(rendered)])
    return "\n".join(lines).strip()
