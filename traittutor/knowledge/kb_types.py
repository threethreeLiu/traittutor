...
lives outside ``data/knowledge_bases`` and we never copy or re-index it. Three
...
across the codebase. ``lightrag_server`` points at no local folder, so
:func:`external_root_of` returns ``None`` for it and retrieval happens over HTTP.

Kept in its own low-level module so both :mod:`traittutor.knowledge.manager`
...
CONNECTED_KB_TYPES = frozenset({OBSIDIAN_KB_TYPE, LINKED_KB_TYPE, LIGHTRAG_SERVER_KB_TYPE})
