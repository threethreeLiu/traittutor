...
class ProgressTracker:
    ...
    def _notify(self, progress: dict):
        ...
        from traittutor.runtime.mode import is_server
    ...
    def _save_progress(self, progress: dict):
        """Save progress to the canonical knowledge state store."""
        try:
            ...
            # Update canonical status and progress.
            ...
            _logger_instance().warning("Failed to save canonical knowledge progress: %s", e)

        try:
            KnowledgeStateStore(self.base_dir).save_progress(self.kb_name, progress)
        except Exception as e:
    ...
    def update(
        ...
        try:
            ...
            fallback_logger = logging.getLogger("traittutor.ProgressTracker")
            prefix = f"[{self.task_id}]" if self.task_id else ""
    ...
    def get_progress(self) -> dict | None:
        ...
        progress = KnowledgeStateStore(self.base_dir).load_progress(self.kb_name)
        if progress:
            return progress
    ...
    def clear(self):
        ...
        KnowledgeStateStore(self.base_dir).delete_progress(self.kb_name)
