...
from traittutor.services.rag.factory import normalize_provider_name
...
class KnowledgeBaseInitializer:
    ...
    def _register_to_config(self) -> None:
        """Register a KB in canonical state with initializing status."""
        try:
    ...
    def _update_metadata_with_provider(self, provider: str) -> None:
        state = KnowledgeStateStore(self.base_dir)
        metadata = state.load_metadata(self.kb_name)
        ...
        state.save_metadata(self.kb_name, metadata)
        ...
        entry = manager.config.setdefault("knowledge_bases", {}).setdefault(
            self.kb_name,
            {"path": self.kb_name, "description": f"Knowledge base: {self.kb_name}"},
        )
        entry["rag_provider"] = provider
    ...
    def create_directory_structure(self) -> None:
        ...
        KnowledgeStateStore(self.base_dir).save_metadata(self.kb_name, metadata)
    ...
    async def display_statistics_generic(self) -> None:
        ...
        from traittutor.services.rag.index_probe import inspect_kb_versions
