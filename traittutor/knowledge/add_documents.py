...
from traittutor.services.rag.factory import has_ready_provider_index, normalize_provider_name
...
def remove_raw_document(kb_dir: Path, file_path: Path) -> RawDocumentRemoval:
    ...
    state = KnowledgeStateStore(kb_dir.parent)
    metadata = state.load_metadata(kb_dir.name)
    hashes = metadata.get("file_hashes")
    was_indexed = isinstance(hashes, dict) and hash_key in hashes
    if isinstance(hashes, dict) and was_indexed:
        del hashes[hash_key]
        state.save_metadata(kb_dir.name, metadata)
...
class DocumentAdder:
    ...
    def __init__(
        ...
        self.state = KnowledgeStateStore(self.base_dir)

        # Incremental adds must use the engine TraitTutor has bound to this KB. An
    ...
    def get_ingested_hashes(self) -> dict[str, str]:
        return self.state.load_metadata(self.kb_name).get("file_hashes", {})
        ...
        for idx, doc_file in enumerate(new_files, 1):
            try:
                if self.progress_tracker is not None:
                    from traittutor.knowledge.progress_tracker import ProgressStage
